...
V1.1 does not promise full autonomy, universal factual accuracy, full Linux product parity, an all-Docker deployment or guaranteed cleanup during simultaneous Mac/network/provider outages. Work Mode permits one exact replacement in one observed existing UTF-8 file; it does not expose file creation, deletion, batching or whole-file fallback. The local model's browser-target selection and exact draft formatting have recorded failures. A reviewed moderate Vitest/mocker advisory remains in development tooling; the vulnerable dev-server path is unused by the prescribed tests. See [the V1.1 release record](docs/history/v1.1/V1.1-RELEASE-COMPLETION.md), [qualification](docs/history/v1/qualification.md), [acceptance](docs/history/v1/acceptance.md), [Sanctum publication checks](docs/history/v1/publication.md) and [dependency review](docs/development/dependency-review.md).
