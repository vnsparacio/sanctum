...
def parse_research(
    payload: dict[str, Any], *, max_sources: int, max_items: int
) -> tuple[list[ResearchSource], list[ProductDiscovery]]:
    ...
    if (
        ...
        or len(payload["sources"]) > max_sources
    ):
    ...
    for raw in payload["sources"]:
        ...
        if published is not None and (
            not isinstance(published, str) or len(published) > 40
        ):
    ...
    if (
        ...
        or len(payload["findings"]) > max_items
    ):
    ...
    for raw in payload["findings"]:
        ...
        if (
            ...
            raise MalformedModelOutput(
                "discovery lacks unique credible source evidence"
            )
        if (
        ...
        discoveries.append(
            ProductDiscovery(
                ...
                _bounded_text(
                    raw["sanctum_connection"], 20, 1200, "Sanctum connection"
                ),
                raw["recommendation"],
                tuple(source_ids),
                tuple(labels),
...
def run_product_scout(
    ...
    *,
    ...
    if mode is RunMode.LIVE and not role.write_enabled:
        raise RuntimeError(
            "Product Scout live writes are disabled until shadow acceptance"
        )
    ...
    with lock.acquired_for(run_id):
        ...
        if fixture_payload is None:
            ...
            raw = fixture_payload
        sources, findings = parse_research(
            raw, max_sources=role.max_sources, max_items=role.max_items
        )
        ...
        artifact = {
            ...
            "findings": [
                {**asdict(item), "fingerprint": item.fingerprint(by_id)}
                for item in accepted
            ],
        ...
        output = (
            prefix
            / ("shadow" if mode is not RunMode.LIVE else "runs")
            / f"{run_id}.json"
        )
        ...
        output.chmod(0o600)
        state_store.save(
            commit=datetime.now(UTC).isoformat(),
