...
def _scoped(path: str, scopes: tuple[str, ...]) -> bool:
    candidate = Path(path)
    return any(
        candidate == Path(scope) or Path(scope) in candidate.parents for scope in scopes
    )
...
def run_repo_steward(
    ...
    *,
    ...
    if mode is RunMode.LIVE and not role.write_enabled:
        raise RuntimeError(
            "Repo Steward live writes are disabled until shadow acceptance"
        )
    ...
    with lock.acquired_for(run_id):
        log.emit(
            "started",
            model=config.model_for(role_name).model,
            reasoning=config.model_for(role_name).reasoning,
        )
        ...
        findings = parse_findings(
            raw, evidence, source="Repo Steward", max_items=role.max_items
        )
        findings, suppressed = suppress_duplicates(
            findings, state.get("fingerprints", [])
        )
        ...
        if mode is RunMode.LIVE:
            ...
            proposals = [
                engineering_finding_proposal(
                    linear_writer.metadata, item, evidence_by_id
                )
        ...
        output.chmod(0o600)
        known = list(state.get("fingerprints", [])) + [
