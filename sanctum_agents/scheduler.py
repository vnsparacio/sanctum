...
def load_schedule_plan(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict) or set(value) != {"schema_version", "timezone", "schedules"}:
    ...
    for item in value["schedules"]:
        required = {"name", "role", "days", "local_time", "command", "enabled", "blocked_by"}
        ...
        names.add(item["name"])
        if not isinstance(item["days"], list) or not item["days"] or any(day not in _DAYS for day in item["days"]):
            raise ScheduleError("schedule days are malformed")
        if not isinstance(item["local_time"], str) or not re.fullmatch(r"(?:[01]\d|2[0-3]):[0-5]\d", item["local_time"]):
            raise ScheduleError("schedule time is malformed")
        if not isinstance(item["command"], list) or any(not isinstance(part, str) or not part for part in item["command"]):
            raise ScheduleError("schedule command is malformed")
        if type(item["enabled"]) is not bool or not isinstance(item["blocked_by"], list):
