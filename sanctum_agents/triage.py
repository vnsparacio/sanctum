...
def load_snapshot(path: Path, *, max_items: int) -> list[TriageIssue]:
    ...
    for raw in payload["issues"]:
        ...
        if any(
            not isinstance(raw[key], str) or not raw[key].strip() for key in text_fields
        ):
...
def parse_decisions(
    payload: dict[str, Any], issues: list[TriageIssue], *, max_items: int
) -> tuple[list[TriageDecision], str]:
    ...
    for raw in raw_decisions:
        ...
        if action == "Duplicate":
            ...
            if duplicate_of is not None:
                raise MalformedModelOutput(
                    "non-duplicate decision has a duplicate target"
                )
            state = {
                "Leave": None,
                "Backlog": "Backlog",
        ...
        decisions.append(
            TriageDecision(
                issue_id, action, reason.strip(), duplicate_of, tuple(related), mutation
            )
        )
