...
def load_packet(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or not isinstance(packet["issue"], dict)
        or not isinstance(packet["pull_request"], dict)
    ):
    ...
    if (
        not isinstance(issue["acceptance_criteria"], list)
        or not issue["acceptance_criteria"]
    ):
    ...
    for criterion in issue["acceptance_criteria"]:
        ...
        if (
            not isinstance(criterion["id"], str)
            or not isinstance(criterion["text"], str)
            or not criterion["text"].strip()
            or criterion["id"] in criterion_ids
        ):
            raise MalformedModelOutput("review criterion identity is malformed")
        criterion_ids.add(criterion["id"])
    if (
        type(pull["number"]) is not int
        or pull["number"] <= 0
    ...
    if (
        not isinstance(pull["changed_files"], list)
        or not pull["changed_files"]
        or any(
            not isinstance(item, str) or item.startswith("/")
            for item in pull["changed_files"]
        )
    ):
        raise MalformedModelOutput("review changed files are malformed")
    if not isinstance(pull["ci"], list) or not isinstance(
        pull["validation_evidence"], list
    ):
        raise MalformedModelOutput("review evidence collections are malformed")
    if (
        not isinstance(packet["relevant_context"], list)
        or len(packet["relevant_context"]) > 20
    ):
    ...
    for item in packet["relevant_context"]:
        if (
            ...
            or len(item["content"]) > 20000
        ):
            raise MalformedModelOutput("review context entry is malformed")
    validate_handoff(
        PullRequestHandoff(
            pull["base_branch"], pull["head_branch"], pull["merged"], issue["state"]
        ),
        issue["identifier"],
    )
    return packet
...
def parse_review(payload: dict[str, Any], packet: dict[str, Any]) -> ReviewResult:
    ...
    if (
        payload["verdict"] not in _VERDICTS
        or not isinstance(payload["summary"], str)
        or not (20 <= len(payload["summary"].strip()) <= 2000)
    ):
    ...
    files = set(packet["pull_request"]["changed_files"]) | {
        item["path"] for item in packet["relevant_context"]
    }
    ...
    def findings(name: str) -> tuple[ReviewFinding, ...]:
        ...
        for raw in raw_items:
            ...
            if (
                ...
                or any(item not in criteria for item in ids)
            ):
            ...
            if raw["line"] is not None and (
                type(raw["line"]) is not int or raw["line"] <= 0 or raw["file"] is None
            ):
                raise MalformedModelOutput("review line is malformed")
            for key, low, high in (
                ("title", 8, 180),
                ("evidence", 20, 1600),
                ("recommendation", 20, 1200),
            ):
                if not isinstance(raw[key], str) or not (
                    low <= len(raw[key].strip()) <= high
                ):
    ...
    return ReviewResult(
        payload["verdict"], payload["summary"].strip(), blocking, nonblocking
    )
...
def run_reviewer(
    ...
    *,
    ...
    log = JsonlRunLog(
        prefix / "logs" / role_name / f"{run_id}.jsonl", run_id, role_name, mode
    )
    lock = ExclusiveRoleLock(
        prefix / "locks" / f"{role_name}.lock", config.runtime["lock_stale_seconds"]
    )
    ...
    with lock.acquired_for(run_id):
        ...
        if mode is RunMode.LIVE:
            raise RuntimeError(
                "Reviewer posting requires live PR and Linear qualification"
            )
        ...
        artifact = {
            ...
            "github_writes": 0,
            "linear_writes": 0,
            "pull_request": packet["pull_request"]["number"],
            "model": model.model,
            "reasoning": model.reasoning,
