...
- V1.2 is released and retained as historical source/runtime evidence. New V1.3 projects start from the updated `v1.3-dev` integration branch on their own `v1.3/project-*` feature branch. Review, test and push that branch, then merge it by PR into `v1.3-dev`. Never start from an unmerged sibling feature branch and never merge automatically.
- For the V1.3 development line, Linear is the work/decision record, GitHub is the source/PR/CI record, Symphony executes only issues in `Ready for Agent` with the `symphony` label, and Sanctum owns policy, prompts, wrappers, tests and guardrails. Management agents may observe and propose but may never set either execution-gate condition, modify code, or merge. Implementation workers stop at `Human Review`; only the owner merges or moves work to `Done`.
- Keep management runs and Symphony bounded by explicit wall-clock, turn, retry and workload limits. A silence timeout or `max_turns` alone is not a sufficient runaway control. The owner-approved V1.3 cap is five implementation workers in one Symphony service; standard and deep services still share one exclusive lock and cannot run together.
- The first gateway-only Splunk APM/custom-metric slice is observational and fail-open for application availability. It may never enter authority, routing, egress, capability, evaluator, verifier or completion decisions. Runpod/vLLM/GPU/host monitoring, collectors, dashboards, detectors, Agent Observability and an Observability Steward remain deferred.
