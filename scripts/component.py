...
p.add_argument(
    "component",
    choices=["mlx", "messages", "gmail", "calendar", "markdown", "files", "webui"],
)
...
if a.health:
    if a.component not in ("mlx", "webui"):
        raise SystemExit("Health probe is supported for mlx/webui only")
    url = (
        f"http://127.0.0.1:{r['mlx_port']}/v1/models"
        if a.component == "mlx"
    ...
    try:
        ...
        if a.component == "webui" and result.get("status") is not True:
...
if a.component in brokers:
    if a.component in ("messages", "gmail", "calendar") and (
        not (prefix / "config" / (a.component + ".enabled")).is_file()
        or (prefix / "config" / (a.component + ".enabled")).read_text().strip()
        != "enabled"
    ):
        raise SystemExit(
    ...
    if not Path(exe).is_file():
        raise SystemExit("Run scripts/bootstrap.py mlx with the same --prefix first")
    if a.cache_only:
    ...
    if not Path(exe).is_file():
        raise SystemExit("Run scripts/bootstrap.py webui with the same --prefix first")
    for name in (
        "ENABLE_OLLAMA_API",
        "ENABLE_OPENAI_API",
        "ENABLE_MEMORIES",
    ...
    args = [exe, "serve", "--host", "127.0.0.1", "--port", "28000"]
os.umask(0o077)
