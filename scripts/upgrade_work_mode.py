...
def atomic(path, data):
    tmp = path.with_name(path.name + ".project3g-tmp")
    op.write(tmp, data)
    os.replace(tmp, path)
...
def safe(prefix):
    if platform.system() != "Darwin":
        raise ValueError("Project 3G live amendment requires macOS")
    if op.owns_process(op.process_record(prefix)):
        raise ValueError("Stop candidate gateway before Work Mode amendment")
    for name in ("gpu.json", "private-lead/gpu.json"):
        ...
        if path.exists():
            ...
            if value.get("phase") != expected_phase or any(
                value.get(k) for k in ("pod_id", "pod_name", "allocation_uncertain")
            ):
                raise ValueError("Unresolved GPU ownership; preserve cleanup")
    retirement = prefix / "state/gate/gpu.json"
    if not retirement.is_file() or not json.loads(retirement.read_text()).get(
        "retired_confirmed_at"
    ):
        raise ValueError("Verify permanent 80B retirement before amendment")
        ...
        if db.exists():
            with sqlite3.connect(db.as_uri() + "?mode=ro", uri=True) as conn:
                if conn.execute("select count(*) from leases").fetchone()[0]:
                    raise ValueError("Close all private leases first")
                if conn.execute(
                    "select count(*) from sqlite_master where type='table' and name='experiments'"
                ).fetchone()[0]:
                    if any(
                        json.loads(row[0])["state"] != "COMPLETE"
                        for row in conn.execute("select record from experiments")
                    ):
                        raise ValueError("Unresolved experiment; preserve supervision")
    plist = prefix / "config/gpu-janitor.plist"
    if not plist.is_file() or plist.is_symlink():
        raise ValueError("Independent janitor configuration unavailable")
    label = json.loads(json.dumps(__import__("plistlib").loads(plist.read_bytes())))[
        "Label"
    ]
    if subprocess.run(
        ["/bin/launchctl", "print", f"gui/{os.getuid()}/{label}"], capture_output=True
    ).returncode:
        raise ValueError("Independent janitor is not loaded")


def docker_details():
    located = shutil.which("docker")
    if not located:
        raise ValueError("Pinned Docker CLI unavailable")
    try:
        docker = str(Path(located).resolve(strict=True))
    except OSError:
        raise ValueError("Pinned Docker CLI unavailable") from None
    if (
        not Path(docker).is_file()
        or Path(docker).is_symlink()
    ...
    if not isinstance(host, str) or not host.startswith("unix://"):
        raise ValueError("Work Mode requires a local Unix Docker endpoint")
    ...
    subprocess.run(
        [
            ...
            tag,
            "--file",
            str(ROOT / "gate/runtime/work-runner.Dockerfile"),
            str(ROOT / "gate/runtime"),
        ],
        check=True,
        timeout=1800,
    )
    ...
    if not image.startswith("sha256:"):
        raise ValueError("Runner image identity unavailable")
...
def rendered_file(prefix, name):
    if name == "SETTINGS.json":
        return rendered_settings(prefix)
    data = (ROOT / "gate" / name).read_text()
    tokens = {
        "@PYTHON@": str(ROOT / ".venv/bin/python"),
        "@STATE@": str(prefix / "state"),
        "@CONFIG@": str(prefix / "config"),
...
def openclaw_config(prefix):
    ...
    entries["workmode-broker"] = {
        "name": "Sanctum Work Mode semantic broker",
...
def work_profile(prefix, docker, host, tag, image):
    ...
    staging = op.private(prefix / "state/gate/private-lead/work-mode/staging")
    op.private(prefix / "state/gate/private-lead/work-mode/tasks")
    op.private(prefix / "state/gate/private-lead/work-mode/workspaces")
    common = {
        "task_protection": unrestricted,
        "disk_bytes": 8 * 1024**3,
        "docker_path": docker,
        "docker_host": host,
        "runner_image": tag,
    ...
    profiles = {
        "sanctum": {
            **common,
            ...
            "operations": {
                **common["operations"],
                "test": ["node", "--test", "gate/tests"],
                "lint": ["node", "--check", "gate/plugin/index.mjs"],
                "build": ["node", "--check", "gate/plugin/index.mjs"],
            },
        }
    }
    fixtures = op.private(prefix / "state/gate/private-lead/work-mode/qualification")
    for number in range(1, 11):
        name = f"grade{number:02d}"
        profiles[name] = {
            **common,
            "task_protection": qualification.protection_contract(name),
            "repository": str(fixtures / name / "repo"),
            "staging_root": str(op.private(fixtures / name / "staging")),
        }
    profiles["adversarial"] = {
        **common,
        "task_protection": qualification.protection_contract("adversarial"),
        "repository": str(fixtures / "adversarial/repo"),
        "staging_root": str(op.private(fixtures / "adversarial/staging")),
        "reviewer": False,
    }
    return (
        json.dumps(
            {"schema": "sanctum-work-mode-profiles/v1", "profiles": profiles}, indent=2
        )
        + "\n"
    )
...
def apply(prefix):
    ...
    print(
        "Applied Project 3G Work Mode amendment. Private rollback record: "
        + str(record)
    )
...
def rollback(prefix, record):
    ...
    print(
        "Restored the pre-Project-3G candidate. Private task receipts and runner image were retained."
    )
...
if __name__ == "__main__":
    ...
    try:
        (
            ...
            if args.apply
            else rollback(args.prefix.absolute(), args.rollback)
        )
