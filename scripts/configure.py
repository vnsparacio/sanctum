...
import argparse,hashlib,importlib.util,json,os,plistlib,re,sqlite3,subprocess,sys,time
...
def integrity_changes(prefix,item):
 if type(item) is not dict or set(item)!={'source_manifest_sha256'} or item['source_manifest_sha256']!=op.sha(ROOT/'SOURCE-MANIFEST.json'):raise ValueError('Integrity amendment must name the exact reviewed source freeze')
 spec=importlib.util.spec_from_file_location('integrity_upgrade',ROOT/'scripts/upgrade_work_mode.py');upgrade=importlib.util.module_from_spec(spec);spec.loader.exec_module(upgrade);upgrade.safe(prefix)
 settings=json.loads((prefix/'gate/SETTINGS.json').read_text())
 if settings['private_lead']['auto_start'] or settings['gpu']['auto_start']:raise ValueError('GPU autostart must remain disabled')
 spec=importlib.util.spec_from_file_location('integrity_qualification',ROOT/'gate/qualify_work_mode.py');qualification=importlib.util.module_from_spec(spec);spec.loader.exec_module(qualification)
 profiles=json.loads((prefix/'config/work-mode.json').read_text());evidence=protection_module()
 if profiles.get('schema')!='sanctum-work-mode-profiles/v1' or not set(qualification.PROTECTED_CASES)<=set(profiles.get('profiles',{})):raise ValueError('Existing qualification profiles required')
 for name,profile in profiles['profiles'].items():
  if name in qualification.PROTECTED_CASES:
   expected=prefix/'state/gate/private-lead/work-mode/qualification'/name/'repo'
   if profile['repository']!=str(expected):raise ValueError('Qualification repository identity mismatch')
   profile['task_protection']=qualification.protection_contract(name)
  else:profile['task_protection']=evidence.validate_contract(profile.get('task_protection',evidence.unrestricted_contract()))
 changes={'gate/'+name:(ROOT/'gate'/name).read_text() for name in INTEGRITY_FILES}
 changes['config/work-mode.json']=json.dumps(profiles,indent=2)+'\n'
 freeze=json.loads((prefix/'gate/FREEZE.json').read_text())
 for name in INTEGRITY_FILES:freeze[name]=hashlib.sha256(changes['gate/'+name].encode()).hexdigest()
 changes['gate/FREEZE.json']=json.dumps(freeze,indent=2)+'\n'
 return changes
...
def configure(prefix,proposal):
 ...
 if 'work_budget' in proposal or 'work_host' in proposal:
  mode='work_budget' if 'work_budget' in proposal else 'work_host';item=proposal[mode]
  fields={'profile','max_iterations','max_model_calls','max_tokens','retained_scope'} if mode=='work_budget' else {'profile','retained_scope','source_manifest_sha256'}
  if set(proposal)!={mode} or type(item) is not dict or set(item)!=fields:raise ValueError('Invalid Work Mode host amendment')
  name=item['profile']
  if type(name) is not str or not re.fullmatch(r'[a-z][a-z0-9_-]{0,31}',name):raise ValueError('Invalid Work Mode profile name')
  if mode=='work_budget':
   for key,limit in (('max_iterations',32),('max_model_calls',32),('max_tokens',200000)):
    if type(item[key]) is not int or not 1<=item[key]<=limit:raise ValueError('Work Mode budget exceeds reviewed ceiling')
  elif item['source_manifest_sha256']!=op.sha(ROOT/'SOURCE-MANIFEST.json'):raise ValueError('Host amendment must name the exact reviewed source freeze')
  profiles=json.loads((prefix/'config/work-mode.json').read_text());current=profiles.get('profiles',{}).get(name)
  if not current or current.get('staging_root')!=str(prefix/'state/gate/private-lead/work-mode/registered'/name/'staging'):raise ValueError('Only separately registered task profiles may be amended')
  settings=json.loads((prefix/'gate/SETTINGS.json').read_text());lead=Path(settings['state_directory'])/'private-lead';state=json.loads((lead/'gpu.json').read_text())
  if settings['private_lead']['auto_start'] or settings['gpu']['auto_start']:raise ValueError('GPU autostart must remain disabled')
  scope=item['retained_scope']
  with sqlite3.connect((lead/'control.sqlite').as_uri()+'?mode=ro',uri=True) as connection:
   leases=connection.execute('select scope,active,closing,expires from leases').fetchall()
  if state.get('pod_id'):
   if type(scope) is not str or not re.fullmatch(r'[a-f0-9]{64}',scope) or state.get('phase')!='READY' or state.get('allocation_uncertain') or len(leases)!=1 or leases[0][0]!=scope or leases[0][1:3]!=(1,0) or leases[0][3]<=time.time() or not active_janitor(prefix):raise ValueError('Retained compute requires exactly one live supervising lease and the independent janitor')
   ps=subprocess.run(['/bin/ps','-axo','command='],capture_output=True,text=True,check=True,timeout=10).stdout
   if any(str(prefix/'gate/worker.py') in line and re.match(r'^\S*python\S*\s+-B\s+',line.strip()) for line in ps.splitlines()):raise ValueError('Inference or capability worker is still active')
  elif scope is not None or leases or state.get('phase')!='OFFLINE' or state.get('allocation_uncertain'):raise ValueError('Unresolved GPU ownership')
  freeze=json.loads((prefix/'gate/FREEZE.json').read_text())
  if mode=='work_budget':
   # Only the reviewed numeric host ceiling may change in a budget amendment.
   target=prefix/'gate/plugin/work-mode.mjs';old=target.read_text();new=(ROOT/'gate/plugin/work-mode.mjs').read_text()
   if old!=new and (old.count('maxIterations>16')!=1 or old.replace('maxIterations>16','maxIterations>32')!=new):raise ValueError('Host budget amendment cannot install unrelated code changes')
   if new.count('maxIterations>32')!=1:raise ValueError('Reviewed host iteration ceiling required')
   changes['gate/plugin/work-mode.mjs']=new
   current.update({key:item[key] for key in ('max_iterations','max_model_calls','max_tokens')});changes['config/work-mode.json']=json.dumps(profiles,indent=2)+'\n'
  else:
  ...
  for name,text in changes.items():
   if name.startswith('gate/'):freeze[name.removeprefix('gate/')]=hashlib.sha256(text.encode()).hexdigest()
  changes['gate/FREEZE.json']=json.dumps(freeze,indent=2)+'\n'
  r['work_mode_source_manifest_sha256']=op.sha(ROOT/'SOURCE-MANIFEST.json')
 if 'work_profile' in proposal:
  item=proposal['work_profile']
  if type(item) is not dict or not {'name','copy_from','repository'}<=set(item) or set(item)-{'name','copy_from','repository','task_protection'}:raise ValueError('Invalid Work Mode profile registration')
  name=item['name'];source=item['copy_from']
  if type(name) is not str or not re.fullmatch(r'[a-z][a-z0-9_-]{0,31}',name) or type(source) is not str:raise ValueError('Invalid Work Mode profile name')
  path=prefix/'config/work-mode.json'
  if path.is_symlink() or not path.is_file() or path.stat().st_mode&0o077:raise ValueError('Private installed Work Mode profiles required')
  profiles=json.loads(path.read_text())
  if profiles.get('schema')!='sanctum-work-mode-profiles/v1' or source not in profiles.get('profiles',{}) or name in profiles['profiles']:raise ValueError('New name and existing reviewed profile required')
  if type(item['repository']) is not str:raise ValueError('Invalid Work Mode repository')
  repo=Path(item['repository']);private=prefix.resolve()
  if not repo.is_absolute() or not repo.is_dir() or repo.resolve()!=repo or not repo.is_relative_to(private) or repo.is_relative_to(ROOT) or repo.stat().st_mode&0o077:raise ValueError('Repository must be a private nonsymlink directory within the owner prefix')
  git_env={'PATH':'/usr/bin:/bin','HOME':'/nonexistent','GIT_CONFIG_NOSYSTEM':'1','GIT_CONFIG_GLOBAL':'/dev/null'}
  top=subprocess.run(['/usr/bin/git','rev-parse','--show-toplevel'],cwd=repo,env=git_env,capture_output=True,text=True,timeout=10)
  clean=subprocess.run(['/usr/bin/git','status','--porcelain=v1','--untracked-files=all'],cwd=repo,env=git_env,capture_output=True,text=True,timeout=10)
  head=subprocess.run(['/usr/bin/git','rev-parse','--verify','HEAD'],cwd=repo,env=git_env,capture_output=True,text=True,timeout=10)
  if top.returncode or top.stdout.strip()!=str(repo) or clean.returncode or clean.stdout or head.returncode:raise ValueError('Repository must have a commit and a clean worktree')
  staging=prefix/'state/gate/private-lead/work-mode/registered'/name/'staging'
  if not staging.is_dir() or staging.resolve()!=staging or staging.stat().st_mode&0o077:raise ValueError('Create the private registered-profile staging directory first')
  profiles['profiles'][name]={**profiles['profiles'][source],'repository':str(repo),'staging_root':str(staging)}
  if 'task_protection' in item:profiles['profiles'][name]['task_protection']=protection_module().validate_contract(item['task_protection'])
  changes['config/work-mode.json']=json.dumps(profiles,indent=2)+'\n'
