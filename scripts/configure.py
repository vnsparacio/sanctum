...
TOOLS={'messages':['messages_chats','messages_history','messages_search'],'gmail':['gmail_search','gmail_read'],'calendar':['calendar_calendars','calendar_events','calendar_event'],'markdown':['save_local_markdown'],'files':['steward_list','steward_inspect','steward_create_folder','steward_move','steward_rename','steward_undo_last'],'browser':['browser']}
...
def configure(prefix,proposal):
 ...
 if type(proposal) is not dict or set(proposal)-{'integrations','contacts','file_roots','accounts','gpu'}:raise ValueError('Unknown configuration field')
 ...
 if 'gpu' in proposal:
  gpu=proposal['gpu'];allowed={'volume_id','datacenter','ssh_private_key','keychain_item','auto_start'}
