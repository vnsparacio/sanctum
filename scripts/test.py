...
def main():
 with tempfile.TemporaryDirectory(prefix='sanctum-tests-') as td:
  ...
  jobs=[('gate',['node','--test','--test-reporter=tap','tests/core.test.mjs','tests/local-agent.test.mjs','tests/source-retrieval.test.mjs']),('gate',[sys.executable,'-B','-m','unittest','discover','-s','tests','-p','test_*.py']),('reliability',['node','--test','--test-reporter=tap',*[str(p.relative_to(ROOT/'reliability')) for p in sorted((ROOT/'reliability/tests').glob('*.test.mjs'))]]),('reliability',[sys.executable,'-B','-m','unittest','discover','-s','tests','-p','test_*.py']),('mcp-integration',['node','--test','--test-reporter=tap','tests/guard.test.mjs']),('.', [sys.executable,'-B','-m','unittest','discover','-s','tests','-p','test_*.py'])]
