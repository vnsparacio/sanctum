...
def reconcile(prefix):
    ...
    if op.owns_process(op.process_record(prefix)):
        raise Refused("gateway_must_be_stopped")
    settings = strict_json((prefix / "gate/SETTINGS.json").read_text())
    if Path(settings["state_directory"]) != prefix / "state/gate":
        raise Refused("retirement_state_binding")
...
if __name__ == "__main__":
    ...
    p.add_argument("--reconcile", action="store_true", required=True)
    args = p.parse_args()
    try:
        print(canonical(reconcile(args.prefix.absolute())))
    except (OSError, ValueError, KeyError):
        raise SystemExit(
            "REFUSED: retirement reconciliation incomplete; preserve ownership and supervision"
        )
