...
def safe_diagnostic(value):
    ...
    allowed = {
        "stage": ("STREAM", "JSON_PARSE", "BACKEND_RESULT"),
    ...
    for key, choices in allowed.items():
        result[key] = value.get(key) if value.get(key) in choices else "UNKNOWN"
    for key in ("stringLength", "unknownFieldCount"):
        number = value.get(key)
        result[key] = (
            min(number, 65537) if type(number) is int and number >= 0 else None
        )
    result["patternMatch"] = (
        value.get("patternMatch") if type(value.get("patternMatch")) is bool else None
    )
...
def rejection(
    ...
    *,
    ...
    diagnostic = {
        "schema": "sanctum-protocol-diagnostic/v1",
        "stage": stage,
        "validatorVersion": {
            "STREAM": "stream/v1",
            "JSON_PARSE": "json/v1",
            "BACKEND_RESULT": "backend-result/v1",
        }[stage],
        "resultKind": (
            kind if kind in ("FINAL", "ESCALATION", "TOOL_PROPOSAL") else "UNKNOWN"
        ),
        "field": "root",
        "keyword": keyword,
        "receivedType": {
        ...
        "parseStatus": (
            "PARSED"
            if parsed
            else "FAILED" if stage == "JSON_PARSE" else "BEFORE_PARSE"
        ),
        "normalization": "NOT_REACHED",
    }
    error = Refused(code)
...
def completion_stream(response):
    ...
    while True:
        try:
            ...
            raise fail("completion") from None
        wire_size += len(raw)
        if len(raw) > 131072 or wire_size > 2097152:
            raise fail("limit")
        try:
            line = raw.decode("utf-8", "strict").strip()
        except UnicodeError:
            raise fail("syntax") from None
        ...
        if data == "[DONE]":
            ...
            break
        try:
            row = finite_json(data)
        except (ValueError, RecursionError) as error:
            raise fail(
                "nonfinite"
                if type(error) is Refused and str(error) == "nonfinite_json"
                else "syntax"
            ) from None
        if type(row) is not dict or row.get("error"):
            raise fail("syntax")
        if type(row.get("usage")) is dict:
            usage = row["usage"]
        ...
        if delta.get("tool_calls") or delta.get("function_call"):
            raise fail("toolCalls")
        if delta.get("refusal"):
            raise fail("refusal")
        piece = delta.get("content")
        if piece is not None and type(piece) is not str:
            raise fail("type")
        if piece:
            if finish is not None:
                raise fail("completion")
            if first is None:
    ...
    return (
        ...
        usage,
        first,
        {"streamStatus": "COMPLETE", "finishStatus": "stop"},
    )
...
def parse_result(text, status):
    try:
        ...
        keyword = (
            {"duplicate_json_key": "duplicateKey", "nonfinite_json": "nonfinite"}.get(
                str(error), "syntax"
            )
