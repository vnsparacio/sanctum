...
def assess(packet, state, audit, strong=False):
    x = validate(audit)
    r = Request(packet['scope'], packet['revision'], (('user', packet['prompt']),), canonical(packet))
    def signal(value, mapping):
        v = mapping[value]
        return Signal(v, r.digest(), ('ambiguity',) if v == 'UNKNOWN' else ())
    d, s = compose(r, TrustedFacts('PERSONAL', True, False, None, True), State(**state),
                   signal(x['urgency'], {'ABSENT':'CLEAR','PRESENT':'POSITIVE','UNKNOWN':'UNKNOWN'}),
                   signal(x['stakes'], {'NORMAL':'CLEAR','HIGH_STAKES':'POSITIVE','UNKNOWN':'UNKNOWN'}))
    needed = x['context_need']['classification']
    if x['urgency'] == 'PRESENT': route = 'URGENT_SAFETY'
    elif 'REQUIRED' in needed.values(): route = 'CONTEXT_REQUIRED'
    elif x['urgency'] == 'UNKNOWN' or x['stakes'] == 'UNKNOWN': route = 'UNAVAILABLE'
    elif d.handling == 'HIGH_STAKES' or strong: route = 'OPENAI_FRONTIER'
    ...
    elif packet['attachment_summary']['visual_count'] and (x['quality']['recommended_tier']=='MULTIMODAL' or x['context_need']['answer']['attachments']=='REQUIRED'): route = 'MULTIMODAL'
    else:
