...
class Request:
    ...
    def valid(self):
        return (
            ...
            and self.revision >= 0
            and type(self.operation_revision) is str
            and type(self.messages) is tuple
            and bool(self.messages)
            and all(
                type(m) is tuple
                and len(m) == 2
                and type(m[0]) is str
                and m[0] in {"user", "assistant", "tool"}
                and type(m[1]) is str
                and bool(m[1])
    ...
    def digest(self):
        return sha256(
            json.dumps(
                [self.scope, self.revision, self.messages, self.operation_revision],
                ensure_ascii=False,
            ).encode()
        ).hexdigest()
...
class Signal:
    ...
    def usable(self, digest):
        return (
            ...
            and self.value in {"CLEAR", "POSITIVE"}
...
def compose(
    request, facts, state, urgency, global_stakes, domain_heads=(), architecture="A"
):
    if type(facts) is not TrustedFacts or type(state) is not State:
        raise TypeError(
            "Trusted Mac facts/state required; learned dictionaries are not accepted"
        )
    ...
    privacy = max(
        (
            v if type(v) is str and v in PRIVACY else "RESTRICTED"
            for v in privacy_values
        ),
        key=PRIVACY.get,
    )
    if not valid:
    ...
    reused_revision = (
        valid
        and state.scope == request.scope
        and request.revision == state.revision
        and (not state.request_digest or state.request_digest != digest)
    )
    ...
    def check(signal):
        ...
        if not signal.usable(digest):
            if type(signal.reasons) is tuple:
                reasons.update(
                    r for r in signal.reasons if type(r) is str and r in REASONS
                )
            reasons.add(
                "ambiguity" if signal.request_digest == digest else "missing_context"
            )
            return "UNKNOWN"
        return signal.value
    ...
    decision = Decision(
        handling, route, privacy, tuple(sorted(reasons)), handling == "NORMAL", external
    )
    ...
    next_state = State(
        request.scope if checkpoint_ok else state.scope,
        handling != "NORMAL" or held,
        privacy,
...
class OfflineRouter:
    ...
    def assess(self, request, facts, state=State()):
        ...
        domains = (
            tuple(call(h) for h in self.domains) if self.architecture == "C" else ()
        )
        return compose(
            request, facts, state, urgency, stakes, domains, self.architecture
        )
