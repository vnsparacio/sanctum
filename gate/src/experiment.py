...
def installed_identity(base=BASE):
    ...
    if not hex_id(source, 64):
        raise Refused("experiment_source_identity")
...
def hex_id(value, size):
    return (
        type(value) is str
        and re.fullmatch("[a-f0-9]{" + str(size) + "}", value) is not None
    )
...
def validate_binding(value):
    if (
        ...
        or not hex_id(value["experiment_id"], 32)
        or any(not hex_id(value[k], 64) for k in ("source_id", "install_id"))
...
class ExperimentLedger:
    def __init__(self, root, now=time.time):
        ...
        with database(self.root) as c:
            c.execute(
                "create table if not exists experiments (id text primary key, record text not null)"
            )
            c.execute(
    ...
    def _read(self, c, experiment_id):
        ...
        if not row:
            raise Refused("experiment_missing")
        return strict_json(row[0])

    def _write(self, c, r):
        c.execute(
            "update experiments set record=? where id=?",
            (canonical(r), r["experiment_id"]),
        )

    def _bound(self, c, binding, identity=None):
        validate_binding(binding)
        r = self._read(c, binding["experiment_id"])
        if any(r[k] != binding[k] for k in IDENTITY_KEYS):
            raise Refused("experiment_stale")
        if identity is not None and any(
            r[k] != identity.get(k) for k in ("source_id", "install_id")
        ):
    ...
    def _current(self, c):
        ...
        if len(open_rows) > 1:
            raise Refused("experiment_ownership_ambiguous")
    ...
    def create(
        self, binding, *, owner_pid, owner_process, expires=None, ordinary_root=None
    ):
        validate_binding(binding)
        now = self.now()
        expires = binding["deadline"] if expires is None else expires
        if (
            not now < expires <= binding["deadline"] <= now + 900
            or type(owner_pid) is not int
            or owner_pid < 1
            or not hex_id(owner_process, 64)
        ):
        ...
        with self.transaction() as c:
            if self._current(c):
                raise Refused("experiment_owned")
            ...
            c.execute(
                ...
                (binding["experiment_id"], canonical(r)),
            )
    ...
    def _admit(self, r):
        ...
        if r["state"] != "ACTIVE":
            raise Refused("experiment_terminal")
    ...
    def check(self, binding, identity=None):
        with self.transaction() as c:
            r = self._bound(c, binding, identity)
            self._admit(r)
            return r
    ...
    def remaining(self, binding, cap, *, cleanup=False):
        with self.transaction() as c:
            ...
            if not cleanup:
                self._admit(r)
            remaining = (
                min(r["expires"], r["deadline"] - (0 if cleanup else CLEANUP_SECONDS))
                - self.now()
            )
    ...
    def reserve(self, binding, identity, call_class, tokens, *, pid=None, process=None):
        if (
            call_class not in ("readiness", "proposal")
        ...
        if not hex_id(process, 64):
            raise Refused("experiment_worker_identity")
        with self.transaction() as c:
            r = self._bound(c, binding, identity)
            self._admit(r)
            if (
                min(r["expires"], r["deadline"] - CLEANUP_SECONDS) - self.now()
                < MIN_CALL_SECONDS
            ):
                raise Refused("experiment_deadline")
            if tokens > r["limits"][call_class + "_tokens"]:
                raise Refused("experiment_token_limit")
            rows = list(
                c.execute(
                    "select class from experiment_attempts where experiment=?",
                    (r["experiment_id"],),
                )
            )
            if len(rows) >= r["limits"]["total"]:
                raise Refused("experiment_total_limit")
            if sum(x[0] == call_class for x in rows) >= r["limits"][call_class]:
                raise Refused("experiment_class_limit")
            number = len(rows) + 1
            c.execute(
                "insert into experiment_attempts values (?,?,?,?,?,NULL,NULL,?,?,?)",
                (
                    r["experiment_id"],
                    number,
                    call_class,
    ...
    def dispatched(self, binding, number):
        ...
        with self.transaction() as c:
            ...
            self._admit(r)
            row = c.execute(
                "select dispatched from experiment_attempts where experiment=? and number=?",
                (r["experiment_id"], number),
            ).fetchone()
            if not row or row[0] is not None:
                raise Refused("experiment_attempt_state")
            c.execute(
                "update experiment_attempts set dispatched=? where experiment=? and number=?",
    ...
    def completion(self, binding, number, known):
        with self.transaction() as c:
            r = self._bound(c, binding)
            c.execute(
                "update experiment_attempts set completed=?,outcome=? where experiment=? and number=? and outcome=?",
                (
                    self.now() if known else None,
                    "KNOWN" if known else "UNCERTAIN",
                    r["experiment_id"],
                    number,
                    "UNCERTAIN",
                ),
    ...
    def stop(self, binding, code):
        if code not in STOP_CODES:
            raise Refused("experiment_stop_code")
        with self.transaction() as c:
            r = self._bound(c, binding)
            if r["state"] != "COMPLETE":
                r.update(
    ...
    def snapshot(self, binding):
        with self.transaction() as c:
            ...
            rows = list(
                c.execute(
                    "select number,class,tokens,reserved,dispatched,completed,outcome,pid,process "
                    "from experiment_attempts where experiment=? order by number",
                    (r["experiment_id"],),
                )
            )
            r["attempts"] = [
            ...
            r["counts"] = {
                "reserved": len(rows),
                "dispatched": sum(x[4] is not None for x in rows),
                "known_completion": sum(x[6] == "KNOWN" for x in rows),
                "uncertain_completion": sum(x[6] == "UNCERTAIN" for x in rows),
            }
            return r
...
class ExperimentDispatch:
    ...
    def timeout(self, cap):
        return self.ledger.remaining(self.binding, cap)

    @contextlib.contextmanager
    def attempt(self, call_class, tokens):
        self.number = self.ledger.reserve(
            self.binding, self.identity, call_class, tokens
        )
        self.known = False
        try:
            ...
            if call_class == "readiness":
                self.ledger.stop(self.binding, "READINESS_FAILED")
            elif not self.known:
                self.ledger.stop(self.binding, "DISPATCH_UNCERTAIN")
            raise
        finally:
