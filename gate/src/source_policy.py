...
SOURCE_REASONS = frozenset({
    'CURRENT_OR_CHANGING', 'TECHNICAL_DOCUMENTATION', 'PRODUCT_OR_MODEL_CAPABILITY',
    'PRICE_OR_AVAILABILITY', 'LAW_OR_REGULATION', 'CURRENT_ENTITY', 'CURRENT_EVENT',
    'RECENT_RESEARCH', 'TRAVEL', 'CURRENT_RECOMMENDATION',
    'NICHE_OR_EXTERNALLY_VERIFIABLE', 'SUPPLIED_EVIDENCE_ADEQUATE',
...
def minimize_query(prompt):
    ...
    return QueryDraft(query, 'PUBLIC', 'PUBLIC_GENERALIZED', reasons, digest({'query': query, 'reasons': reasons}))
...
def decide(packet, audit):
    ...
    if advised != 'WEB_REQUIRED' and UPGRADE.search(prompt):
        need = 'WEB_REQUIRED'; codes = tuple(sorted(set(codes) | {'CURRENT_OR_CHANGING'}))
    if need == 'NONE':
        return SourceDecision('sanctum-source/v1', need, codes, 'NONE', 'NOT_REQUIRED',
                              digest(packet), packet['scope'], packet['revision'])
    draft = minimize_query(prompt)
    return SourceDecision('sanctum-source/v1', need, codes, draft.mode,
                          'REQUIRED' if need == 'WEB_REQUIRED' else 'OPTIONAL',
                          digest(packet), packet['scope'], packet['revision'], query=asdict(draft))
