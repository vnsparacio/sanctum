...
def normalize_image(raw):
    ...
    with Image.open(io.BytesIO(raw)) as source:
        if (
            ...
            raise Refused("unsupported_image")
        source.load()
    ...
    value = out.getvalue()
    if len(value) > 131072:
        raise Refused("normalized_image_too_large")
    return {
        "type": "image_url",
        "image_url": {
            "url": "data:image/jpeg;base64," + base64.b64encode(value).decode()
        },
    }
...
def video_frames(path, cfg):
    ...
    if not match:
        raise Refused("video_duration_unavailable")
    h, m, s = map(float, match.groups())
    duration = h * 3600 + m * 60 + s
    if not 0 < duration <= cfg["max_video_seconds"]:
        raise Refused("video_duration_limit")
    count = min(6, cfg["max_images"])
    times = [round(duration * i / count, 3) for i in range(count)]
    frames = []
    for stamp in times:
        args = [
            ffmpeg,
        ...
        r = subprocess.run(args, capture_output=True, timeout=30)
        if r.returncode or not r.stdout or len(r.stdout) > 2000000:
            raise Refused("video_frame_failed")
        frames.append(normalize_image(r.stdout))
    return frames, {
        "kind": "sampled_video",
        "duration_seconds": duration,
        "frame_timestamps_seconds": times,
...
def prepare(paths, settings, now=time.time):
    if not paths or len(paths) > 12:
        raise Refused("attachment_count_limit")
    ...
    for raw_path in paths:
        ...
        if (
            path.is_symlink()
            or not path.is_file()
            or path.stat().st_size > cfg["max_file_bytes"]
        ):
            raise Refused("unsafe_or_oversize_file")
        ext = path.suffix.lower()
        if ext in [".jpg", ".jpeg", ".png", ".webp"]:
            images.append(normalize_image(path.read_bytes()))
            observations.append({"kind": "image"})
        elif ext in [".mp4", ".mov", ".webm", ".mkv"]:
            # Snapshot the bytes before parsing; no source-file changes during extraction.
            with tempfile.TemporaryDirectory(dir=root) as td:
                copied = Path(td) / ("input" + ext)
            ...
            if ext == ".pdf":
                ...
                if reader.is_encrypted or len(reader.pages) > 50:
                    raise Refused("pdf_limit_or_encrypted")
                value = "\n".join(page.extract_text() or "" for page in reader.pages)
            else:
                value = path.read_text(encoding="utf-8")
            if not value.strip() or len(value.encode()) > settings["max_context_bytes"]:
                raise Refused("document_text_limit")
            documents.append({"kind": "extracted_text", "text": value})
            observations.append(
                {"kind": "document_text", "extraction": "text_only_no_embedded_images"}
            )
        else:
            raise Refused("unsupported_file_type")
    if (
        len(images) > cfg["max_images"]
        or len(canonical(documents).encode()) > settings["max_context_bytes"]
    ):
        raise Refused("attachment_bundle_limit")
    ...
    packet = {
        ...
        "summary": {
            ...
            "video_count": sum(x["kind"] == "sampled_video" for x in observations),
        },
    }
    ...
    return {
        ...
        "command": "/gate attach " + token,
    }


def load(token, expected, scope, settings, bind=False, now=time.time):
    if type(token) is not str or not re.fullmatch("[a-f0-9]{64}", token):
        raise Refused("media_token")
    ...
    if (
        ...
        raise Refused("unsafe_media_snapshot")
    packet = strict_json(path.read_text())
    if digest(packet["content"]) != packet["digest"] or (
        expected and expected != packet["digest"]
    ):
        raise Refused("media_changed")
    with database(settings["state_directory"]) as c:
        c.execute(
            "create table if not exists media_bindings (token text primary key,scope text)"
        )
        row = c.execute(
            "select scope from media_bindings where token=?", (token,)
        ).fetchone()
        if bind and not row:
            if now() - packet["created_at"] > 86400:
                raise Refused("unbound_media_expired")
            c.execute("insert into media_bindings values (?,?)", (token, scope))
            row = (scope,)
        if not row or row[0] != scope:
            raise Refused("media_scope_mismatch")
    return packet
...
def expand(packet, scope, settings):
    ...
    if ref:
        if set(ref) != {"token", "digest"}:
            raise Refused("media_reference")
