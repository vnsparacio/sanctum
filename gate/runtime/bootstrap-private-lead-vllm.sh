...
mkdir -p "$BASE/logs" "$BASE/pids" "$BASE/tmp" "$CACHE"
...
vllm serve "$MODEL" --revision "$REVISION" --host 127.0.0.1 --port 8000 \
  ...
  --quantization modelopt_fp4 --kv-cache-dtype fp8 --gpu-memory-utilization 0.92 \
  --max-model-len 32768 --max-num-seqs 1 --enable-auto-tool-choice \
  --tool-call-parser qwen3_coder --reasoning-parser qwen3 --disable-log-requests \
  --disable-log-stats --enable-prefix-caching --all2all-backend flashinfer_cutlass \
