...
const workModeCapabilities=new Set(['worktree_list','worktree_read','worktree_edit','worktree_patch','worktree_command','source_first_research']);
...
export function capabilityPolicy(name,repairRules=[]){
  ...
  const effect=mutations.has(name)||['worktree_edit','worktree_patch'].includes(name)?'MUTATION':['browser','worktree_command'].includes(name)?'CONTROL':'READ';
