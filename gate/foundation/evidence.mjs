...
export function validateEvidenceItem(v){
 ...
 if(v.fetchStatus==='FETCHED'&&!v.fragments.some(x=>x.kind==='FETCHED_CONTENT'))return {ok:false,code:'EVIDENCE_FETCH_MISSING'};
 if(v.fetchStatus!=='FETCHED'&&v.fragments.some(x=>x.kind==='FETCHED_CONTENT'))return {ok:false,code:'EVIDENCE_FETCH_FALSE'};
...
export function createEvidencePack({requestDigest,scope,revision,sourceNeed,reasonCodes=[],items=[],failureCodes=[],adequacy:state='NOT_REQUIRED',createdAt='1970-01-01T00:00:00Z',budget={candidates:0,fetched:0,chars:0}}={}){
 if(typeof requestDigest!=='string'||!/^[a-f0-9]{64}$/.test(requestDigest)||!text(scope,256)||!Number.isSafeInteger(revision)||revision<0||!needs.has(sourceNeed)||!Array.isArray(reasonCodes)||!reasonCodes.every(code)||!Array.isArray(failureCodes)||!failureCodes.every(code)||!adequacy.has(state)||!Array.isArray(items)||items.length>6||!isRecord(budget)||!Number.isSafeInteger(budget.candidates)||!Number.isSafeInteger(budget.fetched)||!Number.isSafeInteger(budget.chars)||!text(createdAt,64))throw Error('evidence_pack_shape');
...
export function presentEvidence(pack,tier){
 ...
 for(const item of pack.items){if(items.length>=profile.maxSources)break;const fragments=[];for(const f of item.fragments){if(used>=profile.maxChars)break;const text=f.text.slice(0,Math.max(0,profile.maxChars-used));if(text){fragments.push({...f,text});used+=text.length;}}if(fragments.length)items.push({sourceId:item.sourceId,url:item.finalUrl??item.url,title:item.title,fetchStatus:item.fetchStatus,fragments});}
...
export function validateGroundedAnswer(answer,pack){
 ...
 for(const c of answer.citations){if(!exact(c,['sourceId','url'])||!id(c.sourceId)||!url(c.url))return {ok:false,code:'CITATION_SHAPE'};const item=byId.get(c.sourceId);if(!item||item.fetchStatus!=='FETCHED'||(item.finalUrl??item.url)!==c.url)return {ok:false,code:'CITATION_UNDELIVERED'};}
 if(pack?.sourceNeed==='WEB_REQUIRED'&&pack.adequacy==='INADEQUATE'&&answer.grounding==='GROUNDED')return {ok:false,code:'GROUNDING_INADEQUATE'};
