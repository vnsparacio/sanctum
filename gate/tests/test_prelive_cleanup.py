...
class SweepCleanupTests(Fixture):
            ...
            for manual in (False, True):
                with self.subTest(release=cls.__name__, manual=manual):
                    ...
                    with database(lc.root) as c:
                        c.execute(
                            "insert into leases values (?,?,1,0)", ("d" * 32, 999)
                        )
                        c.execute(
                            "insert into leases values (?,?,0,1)", ("e" * 32, 2000)
                        )
                    lc.sweep(manual=manual)
                    with database(lc.root) as c:
                        self.assertEqual(
                            c.execute(
                                "select scope,expires,active,closing from leases order by scope"
                            ).fetchall(),
                            [("d" * 32, 999, 1, 0), ("e" * 32, 2000, 0, 1)],
                        )
                        c.execute(
                            "delete from leases"
                        )  # Explicit synthetic owner reconciliation.
    ...
    def test_80b_sweep_cannot_turn_local_pending_into_confirmed(self):
        ...
        lc.save(
            lc.state(), phase="READY", pod_id="synthetic-pod", pod_name="synthetic-name"
        )
    ...
    def test_sweep_rechecks_experiment_created_while_taking_lifecycle_lock(self):
        for cls in (Private80BLifecycle, PrivateLeadLifecycle):
            with self.subTest(release=cls.__name__):
                ...
                def interleaved_lock(*args, **kwargs):
                    ...
                    with original(*args, **kwargs):
                        binding = {
                            **self.binding,
                            "experiment_id": (
                                "e" if cls is PrivateLeadLifecycle else "f"
                            )
                            * 32,
                        }
                        self.ledger.create(
                ...
                with patch.object(lc, "lock", interleaved_lock):
                    lc.sweep()
                with database(lc.root) as c:
                    self.assertEqual(
                        c.execute("select active from leases").fetchone(), (1,)
                    )
                    c.execute("delete from leases")
                Private80BLifecycle(
                    self.settings, provider=Provider(), now=self.now
                ).sweep()
                PrivateLeadLifecycle(
                    self.settings, provider=Provider(), now=self.now
                ).sweep()
    ...
    def test_ordinary_idle_and_expiry_behavior_without_experiment(self):
        ...
        for cls in (PrivateLeadLifecycle,):
            with self.subTest(release=cls.__name__):
                ...
                with database(lc.root) as c:
                    self.assertEqual(
                        c.execute("select scope,active,closing from leases").fetchall(),
                        [("d" * 32, 0, 0)],
                    )
    ...
    def test_no_admission_transaction_is_held_across_provider_calls(self):
        ...
        for cls in (Private80BLifecycle, PrivateLeadLifecycle):
            ...
            def provider_cleanup(state):
                ...
                t.join(timeout=2)
                self.assertFalse(
                    t.is_alive(), "provider call held admission transaction"
                )

            with patch.object(lc, "terminate_locked", provider_cleanup):
    ...
    def test_identity_match_replacement_and_unavailable_are_distinct(self):
        for observed, expected in (("a" * 64, True), ("b" * 64, False), (None, None)):
            with (
                ...
                patch("experiment_lifecycle.process_identity", return_value=observed),
                patch("experiment_lifecycle.os.kill") as kill,
            ):
                self.assertIs(alive(123, "a" * 64), expected)
                if observed is None:
                    kill.assert_called_once_with(123, 0)
        with (
            patch("experiment_lifecycle.process_identity", return_value=None),
            patch("experiment_lifecycle.os.kill", side_effect=ProcessLookupError),
        ):
            self.assertIs(alive(123, "a" * 64), False)
        for error in (PermissionError(), OSError()):
            with (
                patch("experiment_lifecycle.process_identity", return_value=None),
                patch("experiment_lifecycle.os.kill", side_effect=error),
            ):
    ...
    def test_ps_failure_and_timeout_with_live_process_are_unknown(self):
        ...
        for error in (OSError(), subprocess.TimeoutExpired("ps", 2)):
            with (
                ...
                patch("experiment.subprocess.run", side_effect=error),
            ):
...
class WorkerCleanupTests(Fixture):
    def test_active_diagnostic_row_without_worker_identity_is_preserved(self):
        ...
        lc = PrivateLeadLifecycle(self.settings, provider=Provider(), now=self.now)
        self.assertEqual(
            ExperimentSupervisor(self.ledger, lc).tick()["cleanup"], "LOCAL_PENDING"
        )
        with database(self.state) as c:
            self.assertEqual(c.execute("select active from leases").fetchone(), (1,))
    ...
    def test_worker_and_lease_finalization_commit_or_rollback_together(self):
        ...
        def interrupted(c, r):
            ...
            if r["worker_pid"] is None:
                raise SystemExit("synthetic crash before commit")

        with (
            patch.object(lc.backend, "propose", return_value={"synthetic": True}),
            patch.object(self.ledger, "_write", interrupted),
        ):
            with self.assertRaises(SystemExit):
                diagnostic.propose("d" * 32, {})
        self.assertEqual(self.snapshot()["worker_pid"], os.getpid())
        with database(self.state) as c:
            self.assertEqual(c.execute("select active from leases").fetchone(), (1,))
            c.execute(
                "update leases set active=0"
            )  # Synthetic verified reconciliation before next call.
        with patch.object(lc.backend, "propose", return_value={"synthetic": True}):
            diagnostic.propose("d" * 32, {})
        self.assertIsNone(self.snapshot()["worker_pid"])
        with database(self.state) as c:
            self.assertEqual(c.execute("select active from leases").fetchone(), (0,))
        ...
        for allocation in ("NONE", "OWNED"):
            with (
                ...
                supervisor = ExperimentSupervisor(
                    self.ledger, lc, kill=lambda *args: signals.append(args)
                )
                with patch("experiment_lifecycle.process_identity", return_value=None):
                    result = supervisor.tick()
                self.assertEqual(result["cleanup"], "LOCAL_PENDING")
                ...
                restarted = ExperimentSupervisor(
                    ExperimentLedger(self.state, self.now), lc
                )
        ...
        for role in ("control", "attempt", "allocation"):
            with self.subTest(role=role):
                ...
                if role == "attempt":
                    ...
                    self.update(**{role + "_pid": 123, role + "_process": "a" * 64})
                self.ledger.stop(self.binding, "CANCELLED")
                lc = PrivateLeadLifecycle(
                    self.settings, provider=Provider(), now=self.now
                )
                supervisor = ExperimentSupervisor(
                    self.ledger,
                    lc,
                    is_alive=lambda *_: None,
                    kill=lambda *_: self.fail("unknown identity signaled"),
                )
                self.assertEqual(supervisor.tick()["cleanup"], "LOCAL_PENDING")
                with self.ledger.transaction() as c:
                    c.execute("delete from experiment_attempts")
    ...
    def test_unknown_allocation_worker_does_not_clear_inflight(self):
        ...
        self.assertTrue(result["allocation_inflight"])
...
class Disabled80BTests(Fixture):
    def test_source_default_disables_new_80b_use_before_provider_or_lease(self):
        self.assertIs(
            json.loads((BASE / "SETTINGS.json").read_text())["gpu"]["enabled"], False
        )
        self.update(state="COMPLETE", cleanup="CONFIRMED")
        self.settings["gpu"]["enabled"] = False
        provider = Provider()
        lc = Private80BLifecycle(self.settings, provider=provider, now=self.now)
        for operation in (
            lambda: lc.acquire("d" * 32),
            lambda: lc.infer("d" * 32, {}),
            lambda: lc.propose("d" * 32, {}),
            lambda: lc.ensure_ready("d" * 32, explicit=True),
            lc.resume,
        ):
    ...
    def test_disabled_80b_cleanup_and_worker_refusal_remain_available(self):
        ...
        lc.release("d" * 32, close=True)
        lc.sweep(manual=True)
        self.assertEqual(lc.status()["leases"], 0)
        self.assertEqual(lc.status()["phase"], "RETIRED")
