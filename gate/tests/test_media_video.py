...
from media import load, prepare
...
class Video(unittest.TestCase):
    def test_synthetic_video_has_timestamped_frames_no_audio(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            s = settings(root)
            p = prepare([source], s)
            data = load(p["token"], p["digest"], "a", s, bind=True)
            self.assertEqual(data["summary"]["video_count"], 1)
            self.assertEqual(data["summary"]["visual_count"], 6)
            o = data["content"]["observations"][0]
            self.assertFalse(o["audio_included"])
            self.assertFalse(o["continuous_coverage"])
            self.assertEqual(len(o["frame_timestamps_seconds"]), 6)
