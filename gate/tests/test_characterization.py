...
class Characterization(unittest.TestCase):
    def test_argument_matching_is_partial_but_type_strict(self):
        self.assertTrue(characterization.args_match({"chat_id": 42, "limit": 5}, {"chat_id": 42}))
        self.assertFalse(characterization.args_match({"chat_id": "42"}, {"chat_id": 42}))
        self.assertTrue(characterization.args_match({"query": "Cedar review events"}, {"query": "Cedar review"}))
        self.assertFalse(characterization.args_match({"query": "review events"}, {"query": "Cedar review"}))

    def test_simplest_profile_wins_within_tolerance(self):
        scores = {"compact": 0.98, "contract": 1.0, "contract_examples": 1.0}
        self.assertEqual(characterization.select_simplest(scores, ["compact", "contract", "contract_examples"]), "compact")

    def test_configured_surface_requires_every_schema(self):
        with tempfile.TemporaryDirectory() as directory:
            prefix = Path(directory)
            (prefix / "config").mkdir()
            names = sorted({case["tool"] for case in characterization.TOOL_CASES + characterization.HOLDOUT_CASES})
            (prefix / "config" / "openclaw.json").write_text(json.dumps({"tools": {"alsoAllow": names}}))
    ...
    def test_dynamic_surface_contains_expected_tool(self):
        tools = {name: {"name": name, "description": name, "parameters": {"type": "object"}} for name in characterization.DOMAINS["gmail"]}
        names = characterization.surface_for(characterization.TOOL_CASES[0], "selected", tools)
    ...
    def test_all_expected_arguments_satisfy_captured_top_level_schemas(self):
        schemas = {item["name"]: item["parameters"] for item in json.loads(characterization.schema_path().read_text())}
        for case in characterization.TOOL_CASES + characterization.HOLDOUT_CASES:
            self.assertTrue(characterization.expected_args_valid(case["args"], schemas[case["tool"]]), case["id"])
    ...
    def test_accepted_profile_matches_characterized_prompt(self):
        ...
        self.assertEqual(profile["prompt"]["system"], characterization.SYSTEMS["compact"])
        self.assertEqual(profile["prompt"]["structured_decision_guide"], characterization.DECISION_GUIDE)
        self.assertEqual(profile["tools"]["capability_visibility"], "host_selected_dynamic_subset")
