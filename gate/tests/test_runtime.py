...
def settings(root):
    ...
    s["state_directory"] = str(root)
    s["python"] = sys.executable
    s["gpu"]["auto_start"] = True
    s["gpu"]["enabled"] = True
    return s
...
def audit(tier="LOCAL_4B"):
    return {
        ...
        "uncertainty": [],
        "quality": {"recommended_tier": tier, "reason_codes": ["ROUTINE_LANGUAGE"]},
        "source_need": {
            "classification": "NONE",
            "reason_codes": ["DETERMINISTIC_OR_SELF_CONTAINED"],
        },
        "context_need": {
...
def packet():
    return {
        ...
        "semantic_state": {"high_stakes": False, "privacy_floor": "PERSONAL"},
        "attachment_summary": {
            "count": 0,
            "visual_count": 0,
            "document_count": 0,
...
def chat(text, model=None):
    ...
    if model:
        r.update(model=model["canonical"], provider=model["response_provider"])
    return r
...
class Policy(Temp):
    def test_tiers_independent_of_stakes(self):
        for tier in ["LOCAL_4B", "PRIVATE_80B", "HOSTED_235B", "OPENAI_FRONTIER"]:
            self.assertEqual(
                assess(packet(), state(), audit(tier))["route"],
                "HOSTED_235B" if tier == "PRIVATE_80B" else tier,
            )

    def test_complex_code_uses_eligible_hosted_policy(self):
        a = audit()
        a["quality"]["reason_codes"] = ["TECHNICAL_DEBUGGING"]
        self.assertEqual(assess(packet(), state(), a)["route"], "HOSTED_235B")

    def test_high_stakes_forces_openai(self):
        a = audit()
        a["stakes"] = "HIGH_STAKES"
        self.assertEqual(assess(packet(), state(), a)["route"], "OPENAI_FRONTIER")

    def test_high_stakes_persists(self):
        s = state()
        s["high_stakes"] = True
        self.assertEqual(assess(packet(), s, audit())["route"], "OPENAI_FRONTIER")

    def test_urgency_preempts_frontier(self):
        a = audit()
        a.update(urgency="PRESENT", stakes="HIGH_STAKES")
        self.assertEqual(assess(packet(), state(), a, True)["route"], "URGENT_SAFETY")

    def test_context_classification_blocks(self):
        a = audit()
        a.update(urgency="UNKNOWN", uncertainty=["missing_context"])
        a["context_need"]["classification"]["attachments"] = "REQUIRED"
        self.assertEqual(assess(packet(), state(), a)["route"], "CONTEXT_REQUIRED")

    def test_tools_stay_local(self):
        a = audit("HOSTED_235B")
        a["needs_local_tools"] = True
        self.assertEqual(assess(packet(), state(), a)["route"], "LOCAL_4B")

    def test_visual_capability_route(self):
        p = packet()
        p["attachment_summary"].update(count=1, visual_count=1)
        self.assertEqual(assess(p, state(), audit("MULTIMODAL"))["route"], "MULTIMODAL")

    def test_documents_require_eligible_reasoning(self):
        p = packet()
        p["attachment_summary"].update(count=1, document_count=1)
        a = audit()
        a["context_need"]["answer"]["attachments"] = "REQUIRED"
        self.assertEqual(assess(p, state(), a)["route"], "HOSTED_235B")

    def test_irrelevant_attachment_does_not_escalate(self):
        p = packet()
        p["attachment_summary"].update(count=1, visual_count=1)
        self.assertEqual(assess(p, state(), audit())["route"], "LOCAL_4B")
...
class Authority(Temp):
    def setUp(self):
        ...
        (self.root / "authority.key").write_bytes(self.key)
    ...
    def body(self):
        return {
            ...
            "state": state(),
            "scope": "a" * 32,
            "approval": "session_private_prompt",
            "strong": False,
    ...
    def auth(self, b):
        return authorize(self.signed(b), self.s, now=lambda: 1000, settings_hash="spec")

    def test_once_only_durable_across_calls(self):
        b = self.body()
        b.update(tier="HOSTED_235B", approval="exact_disclosure")
        self.auth(b)
    ...
    def test_disabled_80b_refuses_both_inference_approval_classes(self):
        ...
        b = self.body()
        b.update(
            operation="status", tier="CONTROL", approval="local_control", packet={}
        )
        self.auth(b)

    def test_wrong_signature(self):
        e = self.signed(self.body())
        e["body"] += " "
        with self.assertRaises(Refused):
            authorize(e, self.s, now=lambda: 1000, settings_hash="spec")
    ...
    def test_session_grant_never_history(self):
        b = self.body()
        b["packet"]["history"] = [{"role": "assistant", "content": "private tool data"}]
        with self.assertRaises(Refused):
            self.auth(b)
    ...
    def test_private_lead_proposal_requires_exact_signed_scope(self):
        ...
        self.auth(b)
        for key, value in [
            ("approval", "exact_disclosure"),
            ("tier", "PRIVATE_80B"),
            ("packet", {"request": {"system": "x", "request": {}}, "extra": True}),
        ]:
            bad = self.body()
            bad.update(
                operation="private_lead_propose",
...
class Transport(Temp):
    def test_minimal_classifier_packet_and_provider_policy(self):
        ...
        def send(url, p, headers, **kw):
            sent.append(p)
            return chat(canonical(audit()), self.s["models"]["GEMINI_AUDIT"])

        Remote(self.s, send, lambda: "test").classify(packet(), "one", "audit only")
        p = sent[0]
        self.assertEqual(len(p["messages"]), 2)
        self.assertNotIn("history", canonical(p))
        self.assertNotIn("tools", p)
        self.assertTrue(p["provider"]["zdr"])
    ...
    def test_frontier_is_openai_only(self):
        ...
        def send(url, p, headers, **kw):
            sent.append((url, p))
            return chat(
                canonical({"answer": "test", "escalation": "NONE"}),
                self.s["models"]["OPENAI_FRONTIER"],
            )

        Remote(self.s, send, lambda: "test").infer(
            "OPENAI_FRONTIER", {"prompt": "synthetic"}, "two"
        )
        self.assertEqual(sent[0][1]["provider"]["only"], ["azure"])
        self.assertNotIn("tools", sent[0][1])

    def test_direct_openai_store_false(self):
        self.s["frontier_transport"] = "openai"
        self.s["direct_openai_retention_authorized"] = True
        sent = []
                ...
                "output": [
                    {
                        ...
                        "content": [
                            {
                                "type": "output_text",
                                "text": '{"answer":"test","escalation":"NONE"}',
                            }
                        ],
                    }
                ],
        ...
        Remote(self.s, send, direct_key=lambda: "test").infer(
            "OPENAI_FRONTIER", {"prompt": "synthetic"}, "direct"
        )
        self.assertEqual(sent[0][0], "https://api.openai.com/v1/responses")
        self.assertFalse(sent[0][1]["store"])
        self.assertNotIn("tools", sent[0][1])
    ...
    def test_no_retry_and_budget_reserved_on_timeout(self):
        ...
        with self.assertRaises(Refused):
            Remote(self.s, fail, lambda: "test").infer(
                "HOSTED_235B", {"prompt": "synthetic"}, "timeout"
            )
        self.assertEqual(len(calls), 1)
        with database(self.root) as c:
            self.assertGreater(
                c.execute("select charged from network").fetchone()[0], 0
            )

    def test_budget_blocks_before_network(self):
        self.s["max_request_usd"] = 0
        with self.assertRaises(Refused):
            Remote(self.s, lambda *a, **k: self.fail("network")).infer(
                "HOSTED_235B", {"prompt": "test"}, "budget"
            )

    def test_wrong_provider_and_tools_rejected(self):
        m = self.s["models"]["HOSTED_235B"]
        r = chat("ok", m)
        r["provider"] = "Other"
        with self.assertRaises(Refused):
            extract_chat(r, m)
        r = chat("ok")
        r["choices"][0]["message"]["tool_calls"] = [{"name": "shell"}]
        with self.assertRaises(Refused):
            extract_chat(r)

    def test_text_only_rejects_images(self):
        with self.assertRaises(Refused):
            Remote(self.s).infer("HOSTED_235B", {"prompt": "x", "images": [{}]}, "bad")
        with self.assertRaises(Refused):
            Private80BBackend(self.s).infer({"prompt": "x", "images": [{}]})

    def test_80_readiness_requires_alias_and_smoke(self):
        def send(url, p=None, *args, **kw):
            return (
                {"data": [{"id": "vinceai-qwen80b"}]}
                if url.endswith("/models")
                else chat("READY")
            )

        with self.assertRaisesRegex(Refused, "private_80b_retired"):
            Private80BBackend(self.s, send).health_check(smoke=True)
        with self.assertRaises(Refused):
            Private80BBackend(
                self.s, lambda *a, **k: {"data": [{"id": "wrong"}]}
            ).health_check()
    ...
    def test_private_lead_has_its_own_alias_and_loopback_port(self):
        ...
        def send(url, p=None, *args, **kw):
            if not url.endswith("/models"):
                calls.append(p)
            return (
                {"data": [{"id": "sanctum-private-lead-qwen35-122b"}]}
                if url.endswith("/models")
                else chat("READY")
            )
    ...
    def test_private_lead_proposal_is_data_only_and_thinking_off(self):
        self.s["private_lead"]["enabled"] = True
        calls = []
        response = {"kind": "FINAL", "text": "synthetic"}

        def send(url, p=None, *args, **kw):
            if url.endswith("/models"):
                return {"data": [{"id": "sanctum-private-lead-qwen35-122b"}]}
            calls.append(p)
            return chat(canonical(response))
        ...
        result = PrivateLeadBackend(self.s, send).propose(
            {"system": "synthetic", "request": {"state": {"workIntent": intent}}}
        )
        self.assertEqual(result["status"], "OK")
        self.assertEqual(result["result"], response)
        self.assertEqual(result["telemetry"]["result_kind"], "FINAL")
        self.assertIn("elapsed_seconds", result["telemetry"])
        self.assertEqual(calls[0]["chat_template_kwargs"], {"enable_thinking": False})
        ...
        self.assertEqual(structured_http_reason(400), "structured_decoding_http_400")
        self.assertEqual(structured_http_reason(422), "structured_decoding_http_422")
        self.assertEqual(structured_http_reason(503), "http_503")

    def test_private_lead_proposal_refuses_invalid_request(self):
        with self.assertRaises(Refused):
            PrivateLeadBackend(self.s).propose({"request": {}})
    ...
    def test_private_lead_accepts_exact_production_preflight_schema(self):
        ...
        document = json.loads(
            subprocess.check_output(
                ["node", str(BASE / "preflight-work-intent.mjs"), "--json"], text=True
            )
        )
        intent = document["schemas"]["ordinaryEligible"]["request"]

        def send(url, p=None, *args, **kwargs):
            if url.endswith("/models"):
                return {"data": [{"id": "sanctum-private-lead-qwen35-122b"}]}
            captured.append(p)
            return chat(canonical({"kind": "FINAL", "text": "READY"}))
        ...
        self.assertEqual(result["status"], "OK")
        self.assertEqual(
            captured[0]["response_format"]["json_schema"]["schema"], intent["schema"]
        )
        self.assertRegex(intent["schemaDigest"], r"^[a-f0-9]{64}$")
    ...
    def test_grounded_answer_schema_is_selected_for_profiled_evidence(self):
        ...
        def send(url, p, headers, **kw):
            sent.append(p)
            return chat(canonical(grounded), self.s["models"]["HOSTED_235B"])

        result = Remote(self.s, send, lambda: "test").infer(
            "HOSTED_235B",
            {"prompt": "current", "evidence": {"profile": "HOSTED_RICH"}},
            "grounded",
        )
        self.assertEqual(result["grounded"], grounded)
        self.assertEqual(
            sent[0]["response_format"]["json_schema"]["schema"]["properties"]["kind"][
                "enum"
            ],
            ["GROUNDED_FINAL"],
        )
...
class FakeProvider:
    ...
    def preflight(self):
        return {"available": self.available, "hourly_usd": 2}

    def create(self, name):
        self.created += 1
        if self.fail_create:
            raise Refused("uncertain")
    ...
    def ssh_info(self, pod):
        return ("127.0.0.2", 22)

    def ssh(self, *args):
        pass

    def bootstrap_server(self, *args):
        self.booted = True
        self.calls.append("bootstrap")

    def server_alive(self, *args):
        if not self.booted:
            raise Refused("server_down")

    def tunnel(self, *args):
        self.calls.append("tunnel")

    def close_tunnel(self, *args):
        self.calls.append("tunnel_close")
...
class FakeBackend:
    ...
    def infer(self, p):
        ...
        return {"status": "OK", "text": "synthetic", "escalation": "NONE"}

    def propose(self, p):
        self.calls.append("propose")
        return {"status": "OK", "result": {"kind": "FINAL", "text": "synthetic"}}


class Lifecycle(Temp):
    def setUp(self):
        super().setUp()
        self.s["private_lead"]["auto_start"] = True
        self.time = 1000
        self.p = FakeProvider()
        self.b = FakeBackend()
        self.lc = PrivateLeadLifecycle(
            self.s, self.p, self.b, lambda: self.time, self.advance
        )
    ...
    def test_capacity_rejection_is_bounded_and_cancellable(self):
        ...
        self.p.create = create
        with self.assertRaisesRegex(Refused, "capacity_timeout"):
            self.lc.infer("a", {})
        self.assertFalse(self.lc.state()["allocation_uncertain"])
        self.assertIsNone(self.lc.state()["pod_id"])
        self.lc.sweep(manual=True)
        self.assertEqual(self.lc.status()["phase"], "OFFLINE")
        with self.assertRaisesRegex(Refused, "manual_stop"):
            self.lc.infer("a", {})

    def test_only_exact_create_capacity_rejection_is_retryable(self):
        e = {
            "code": "graphql_error",
            "error": "failed to create pod: graphql error: There are no longer any instances available with the requested specifications. Please refresh and try again.",
        }
        self.assertTrue(capacity_rejected(["pod", "create"], e))
        self.assertFalse(capacity_rejected(["pod", "list"], e))
        self.assertFalse(
            capacity_rejected(
                ["pod", "create"], {**e, "error": "timeout: " + e["error"]}
            )
        )
        self.assertFalse(capacity_rejected(["pod", "create"], None))

    def test_create_ready_reuse_close_preserves_volume(self):
        self.lc.infer("a", {"prompt": "one"})
        self.lc.infer("a", {"prompt": "two"})
        self.assertEqual(self.p.created, 1)
        self.assertEqual(self.lc.status()["phase"], "READY")
        self.lc.release("a", close=True)
        self.assertEqual(self.p.deleted, ["pod1"])
        self.assertEqual(self.lc.status()["phase"], "OFFLINE")
        self.assertNotIn("delete_volume", dir(self.p))

    def test_new_allocation_clears_prior_allocation_telemetry(self):
        self.lc.save(
            self.lc.state(),
            ready_at=1,
            first_inference_at=2,
            last_inference_at=3,
            delete_requested_at=4,
            absent_confirmed_at=5,
            allocation_id="old",
        )
        self.lc.infer("a", {})
        state = self.lc.state()
        self.assertEqual(state["allocation_id"], "pod1")
        self.assertIsNone(state["delete_requested_at"])
        self.assertIsNone(state["absent_confirmed_at"])
        self.assertEqual(state["ready_at"], self.time)
        self.assertIsNone(state["first_inference_at"])
        self.assertIsNone(state["last_inference_at"])
    ...
    def test_readiness_failure_never_queries_and_cleans(self):
        self.b.fail = True
        self.s["private_lead"]["readiness_seconds"] = 10
        with self.assertRaises(Refused):
            self.lc.infer("a", {})
        self.assertNotIn("infer", self.b.calls)
        self.assertEqual(self.p.deleted, ["pod1"])

    def test_delete_unconfirmed_not_offline(self):
        self.lc.infer("a", {})
        self.p.fail_delete = True
        with self.assertRaises(Refused):
            self.lc.release("a", close=True)
        self.assertEqual(self.lc.status()["phase"], "DEGRADED")
        self.assertEqual(self.lc.state()["pod_id"], "pod1")
    ...
    def test_external_delete_disables_restart(self):
        ...
        with self.assertRaises(Refused):
            self.lc.infer("a", {})
        self.assertTrue(self.lc.state()["manual_stop"])
        self.assertEqual(self.p.created, 1)
    ...
    def test_lead_refuses_coexisting_80b_pod(self):
        ...
        lead = PrivateLeadLifecycle(
            self.s, self.p, self.b, lambda: self.time, self.advance
        )
        with self.assertRaisesRegex(Refused, "untracked_or_duplicate_pod"):
            lead.infer("lead", {})

    def test_explicit_signed_lead_proposal_can_start_while_background_autostart_stays_off(
        self,
    ):
        self.s["private_lead"]["enabled"] = True
        self.s["private_lead"]["auto_start"] = False
        lead = PrivateLeadLifecycle(
            self.s, self.p, self.b, lambda: self.time, self.advance
        )
        self.assertEqual(lead.propose("lead", {})["status"], "OK")
        self.assertEqual(self.p.created, 1)
        self.assertIn("propose", self.b.calls)


class Janitor(Temp):
    def test_private_lead_resume_preflight_uses_production_schema(self):
        result = manage.work_intent_preflight()
        eligible = result["schemas"]["ordinaryEligible"]
    ...
    def test_default_sweep_attempts_both_releases_without_one_masking_the_other(self):
        ...
        class Fake:
            ...
            def sweep(self):
                ...
                if self.release == "PRIVATE_80B":
                    raise Refused("other_release_active")

            def status(self):
                return {"phase": "READY"}

        original = manage.lifecycle
        manage.lifecycle = lambda _settings, release: Fake(release)
        self.addCleanup(setattr, manage, "lifecycle", original)
        result = manage.sweep_all(self.s)
        self.assertEqual(calls, list(manage.RELEASES))
        self.assertEqual(result["PRIVATE_80B"]["phase"], "UNAVAILABLE")
        self.assertEqual(result["PRIVATE_LEAD"]["phase"], "READY")

    def test_default_sweep_fails_if_neither_release_can_be_reconciled(self):
        class Fake:
            def sweep(self):
                raise Refused("provider_unavailable")

        original = manage.lifecycle
        manage.lifecycle = lambda _settings, _release: Fake()
        self.addCleanup(setattr, manage, "lifecycle", original)
        with self.assertRaisesRegex(Refused, "janitor_all_releases_failed"):
            manage.sweep_all(self.s)
...
class Media(Temp):
    def test_image_strips_exif_and_no_source_path(self):
        ...
        path = self.root / "private-name.jpg"
        im = Image.new("RGB", (20, 20), "red")
        exif = Image.Exif()
        exif[270] = "secret metadata"
        im.save(path, exif=exif)
        p = prepare([path], self.s)
        m = load(p["token"], p["digest"], "scope", self.s, bind=True)
        self.assertNotIn("private-name", canonical(m))
        self.assertNotIn("secret metadata", canonical(m))
        self.assertEqual(m["summary"]["visual_count"], 1)

    def test_scope_binding_and_hash_drift(self):
        path = self.root / "source.txt"
        path.write_text("synthetic")
        p = prepare([path], self.s)
        load(p["token"], p["digest"], "a", self.s, bind=True)
        with self.assertRaises(Refused):
            load(p["token"], p["digest"], "b", self.s, bind=True)
        with self.assertRaises(Refused):
            load(p["token"], "wrong", "a", self.s)
    ...
    def test_expansion_is_explicit_only(self):
        ...
        e = expand(
            {
                "prompt": "test",
                "media_ref": {"token": p["token"], "digest": p["digest"]},
            },
            "a",
            self.s,
        )
        self.assertEqual(e["documents"][0]["text"], "private document")
        self.assertNotIn("token", e)

    def test_unsupported_and_symlink(self):
        path = self.root / "source.html"
        path.write_text("<script>bad</script>")
        with self.assertRaises(Refused):
            prepare([path], self.s)
...
class Installer(Temp):
    def setUp(self):
        ...
        self.db = self.root / "webui.db"
        self.launch = self.root / "launch.plist"
        ...
        for name, (relative, _) in previous_functions.items():
            previous = self.root / (name + ".py")
            previous.write_text("# synthetic previous " + name + "\n")
            install.FUNCTIONS[name] = (relative, previous)
        self.config.write_text(
            json.dumps(
                {
                    "plugins": {
                        "load": {"paths": [install.OLD, "untouched"]},
                        "entries": {"hybrid-ai-prompt-gate": {"enabled": True}},
                    },
                    "secret": "fixture",
                }
            )
        ...
        with sqlite3.connect(self.db) as c:
            c.execute(
                "create table function(id text primary key,content text,is_active integer,updated_at integer)"
            )
            for name, (_, previous) in install.FUNCTIONS.items():
                c.execute(
                    "insert into function values (?,?,1,123)",
                    (name, previous.read_text()),
                )

    def do_install(self, hook=lambda _: None):
    ...
    def rollback(self):
        return install.rollback(
            self.inst, runner=lambda *a: None, gpu_check=lambda: None
        )
    ...
    def test_install_and_rollback_preserves_unrelated(self):
        ...
        cfg["new_setting"] = 42
        cfg["plugins"]["load"]["paths"].append("another")
        self.config.write_text(json.dumps(cfg))
        self.rollback()
        cfg = json.loads(self.config.read_text())
        self.assertEqual(cfg["new_setting"], 42)
        self.assertEqual(cfg["secret"], "fixture")
        self.assertIn("another", cfg["plugins"]["load"]["paths"])
        self.assertIn(install.OLD, cfg["plugins"]["load"]["paths"])
        ...
        for boundary in [
            ...
            with self.subTest(boundary=boundary):
                ...
                with self.assertRaises(RuntimeError):
                    self.do_install(fail)
                self.rollback()
                self.assertIn(
                    install.OLD,
                    json.loads(self.config.read_text())["plugins"]["load"]["paths"],
                )

    def test_baseline_drift_refuses_before_mutation(self):
        with sqlite3.connect(self.db) as c:
            c.execute("update function set content='changed'")
    ...
    def test_rollback_refuses_changed_gate(self):
        self.do_install()
        with sqlite3.connect(self.db) as c:
            c.execute("update function set content='new user change'")
        with self.assertRaises(Refused):
            self.rollback()
    ...
    def test_rollback_keeps_janitor_when_compute_uncertain(self):
        ...
        with self.assertRaises(Refused):
            install.rollback(self.inst, gpu_check=refuse)
...
class IsolatedTunnel(Temp):
    def test_private_lead_guard_uses_release_specific_state_root(self):
        ...
        self.assertEqual(
            Runpod(self.s, self.s["private_lead"]).guard_root(),
            self.root / "private-lead",
        )
    ...
    def test_private_backend_and_ssh_use_the_same_loopback_port(self):
        ...
        with (
            patch.object(
                provider, "socket_path", return_value=str(self.root / "test.sock")
            ),
            patch.object(provider, "ssh_args", return_value=["ssh"]),
            patch(
