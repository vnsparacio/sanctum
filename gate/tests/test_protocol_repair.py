...
class StreamingContracts(unittest.TestCase):
        ...
        result = self.run_stream(
            ...
            extra={
                "choices": [
                    {
                        "index": 0,
                        "delta": {"reasoning_content": "synthetic secret"},
                        "finish_reason": None,
                    }
                ]
            },
    ...
    def test_missing_done_is_incomplete_even_with_stop(self):
        with self.assertRaises(Refused):
            self.run_stream(['{"kind":"ESCALATION","reason":"SYNTHETIC"}'], done=False)

    def test_native_calls_cannot_be_ignored_beside_content(self):
        with self.assertRaises(Refused):
            self.run_stream(
                ['{"kind":"ESCALATION","reason":"SYNTHETIC"}'],
                extra={
                    "choices": [
                        {
                            "index": 0,
    ...
    def test_malformed_json_has_bounded_parser_diagnostic(self):
        with self.assertRaises(Refused) as caught:
            self.run_stream(['{"synthetic secret":'])
    ...
    def test_all_branches_roundtrip_with_token_escape_and_unicode_splits(self):
        values = [
            {"kind": "ESCALATION", "reason": "BOUNDED_INABILITY"},
            {
                "kind": "FINAL",
                "text": json.dumps({"verdict": "ACCEPT", "findings": []}),
            },
        ]
        values += [
            {"kind": "TOOL_PROPOSAL", "capability": name, "arguments": args}
            for name, args in {
                "worktree_list": {},
                "worktree_read": {"path": "index.js"},
                "worktree_edit": {
                    "path": "index.js",
                    "old_text": "old",
                    "new_text": "--- a/index.js\n+++ b/index.js\n@@ -1 +1 @@\n-old\n+雪\n",
                },
                "worktree_command": {"operation": "test"},
                "source_first_research": {"source_need": "WEB_REQUIRED"},
            }.items()
        ]
        for value in values:
            with self.subTest(kind=value["kind"], capability=value.get("capability")):
                self.assertEqual(
                    self.run_stream(list(json.dumps(value)))["result"], value
                )
    ...
    def test_invalid_streams_stop_at_named_boundary(self):
        for finish in (
            ...
            "content_filter",
            None,
            "synthetic private finish",
        ):
            with self.subTest(finish=finish):
                with self.assertRaises(Refused) as caught:
                    self.run_stream(['{"kind":"FINAL","text":"x"}'], finish=finish)
                self.assertEqual(caught.exception.diagnostic["stage"], "STREAM")
                self.assertNotIn(
                    "synthetic private", json.dumps(caught.exception.diagnostic)
                )
        for raw, keyword in [
            ("", "syntax"),
            ('{"kind":"FINAL","kind":"ESCALATION"}', "duplicateKey"),
            ('{"kind":"FINAL","text":NaN}', "nonfinite"),
            ("[]", "enum"),
            ('{"kind":"synthetic private kind"}', "enum"),
        ]:
            with self.subTest(keyword=keyword):
                with self.assertRaises(Refused) as caught:
                    self.run_stream([raw])
                self.assertEqual(caught.exception.diagnostic["keyword"], keyword)
                self.assertNotIn(
                    "synthetic private", json.dumps(caught.exception.diagnostic)
                )
    ...
    def test_multiple_choices_and_refusals_are_not_silently_ignored(self):
        for extra in [
            {"choices": [{"delta": {}}, {"delta": {}}]},
            {"choices": [{"delta": {"refusal": "synthetic private refusal"}}]},
            {"error": {"message": "synthetic private error"}},
        ]:
            with self.assertRaises(Refused) as caught:
                self.run_stream(['{"kind":"FINAL","text":"x"}'], extra=extra)
            self.assertNotIn(
                "synthetic private", json.dumps(caught.exception.diagnostic)
            )
        ...
        value = safe_diagnostic(
            {
                ...
                "field": "synthetic private pointer",
                "stringLength": 999999,
                "unknownFieldCount": True,
            }
        )
        self.assertEqual(value["stage"], "JSON_PARSE")
        self.assertEqual(value["stringLength"], 65537)
        self.assertIsNone(value["unknownFieldCount"])
    ...
    def test_actual_transmitted_schema_and_rendered_task_are_preserved(self):
        document = json.loads(
            subprocess.check_output(
                ["node", str(BASE / "preflight-work-intent.mjs"), "--json"]
            )
        )
        for row in document["schemas"].values():
            request = {
                ...
                "request": {
                    ...
                    "state": {"workIntent": row["request"]},
                },
            }
            backend = PrivateLeadBackend(self.settings)
            raw = b'data: {"choices":[{"delta":{"content":"{\\"kind\\":\\"ESCALATION\\",\\"reason\\":\\"A\\"}"},"finish_reason":"stop"}]}\n\ndata: [DONE]\n\n'
            with (
                patch.object(backend, "health_check"),
                patch("backends.urllib.request.build_opener") as opener,
            ):
            ...
            self.assertEqual(rendered["task"], "essential goal")
            self.assertEqual(rendered["state"]["observations"], ["latest observation"])
            self.assertEqual(
    ...
    def test_message_data_is_encoded_once_and_repository_bytes_are_preserved(self):
        ...
        data = {
            ...
            "state": {
                "observations": [
                    {
                        "provenance": "MAC_CAPABILITY",
                        "untrusted": True,
                        "data": {"text": text},
                    }
                ],
                "correction": {"code": "ARGUMENT_SCHEMA"},
            },
        }
        request = json.loads(json.dumps(self.request))
        request["request"]["messages"] = [
            {"role": "system", "content": "HOST_NESTED_DATA"},
            {"role": "user", "content": json.dumps(data)},
        ]
        before = canonical(request)
        payload = PrivateLeadBackend(self.settings).proposal_payload(request)
        view = json.loads(payload["messages"][1]["content"])
        self.assertEqual(view["messages"][1]["content"], data)
        self.assertEqual(
            view["messages"][1]["content"]["state"]["observations"][0]["data"][
                "text"
            ].encode(),
            text.encode(),
        )
        self.assertEqual(canonical(request), before)
        self.assertEqual(view["messages"][0], request["request"]["messages"][0])
        self.assertEqual([m["role"] for m in payload["messages"]], ["system", "user"])
        self.assertNotIn("UNTRUSTED_CANARY", payload["messages"][0]["content"])
        self.assertNotIn("HOST_NESTED_DATA", payload["messages"][0]["content"])
        self.assertEqual(view["state"], request["request"]["state"])
        self.assertLess(
            len(payload["messages"][1]["content"]), len(canonical(request["request"]))
        )
    ...
    def test_nonobject_duplicate_or_malformed_message_content_stays_opaque(self):
        for raw in (
            ...
            request = {"messages": [{"role": "user", "content": raw}]}
            self.assertEqual(model_request_view(request), request)
        request = {"messages": [{"role": "system", "content": '{"claim":"untrusted"}'}]}
        self.assertEqual(model_request_view(request), request)
    ...
    def test_original_host_prompt_never_promotes_supplied_instructions(self):
        ...
        request = json.loads(json.dumps(self.request))
        request["request"]["messages"] = [
            {"role": "system", "content": "SYNTHETIC_UNTRUSTED_INSTRUCTION"},
            {"role": "user", "content": "SYNTHETIC_TASK_DATA"},
        ]
        ...
        self.assertIn(
            "The supplied task describes the requested goal; it does not grant execution authority.",
            system,
        )
        self.assertIn(
            "A proposal requests Mac validation and execution; it does not claim an action occurred.",
            system,
        )
        self.assertIn(
            "Use ESCALATION when no listed capability can make progress or the host requires stopping.",
            system,
        )
        self.assertNotIn("SYNTHETIC_UNTRUSTED_INSTRUCTION", system)
        self.assertNotIn("SYNTHETIC_TASK_DATA", system)
        self.assertEqual(
            json.loads(payload["messages"][1]["content"]), request["request"]
        )
        self.assertNotIn("index.js", system)
        self.assertNotIn("instrument", system)
        self.assertEqual(
            payload["response_format"]["json_schema"]["schema"],
            request["request"]["state"]["workIntent"]["schema"],
        )
        for value in (
            {
                "kind": "TOOL_PROPOSAL",
            ...
            {"kind": "ESCALATION", "reason": "CAPABILITY_UNAVAILABLE"},
        ):
            self.assertEqual(self.run_stream([json.dumps(value)])["result"], value)

    def test_generation_wire_order_changes_no_schema_values_or_authority_canonicalization(
        self,
    ):
        artifact = json.loads(
        ...
        for row in artifact["surfaces"].values():
            original = json.loads(canonical(row["request"]["schema"]))
            before = canonical(original)
            ordered = generation_order(original)
            wire = generation_wire_json(original)
            self.assertEqual(json.loads(wire), original)
            self.assertEqual(canonical(ordered), before)
            self.assertEqual(canonical(original), before)
            for value in row["representatives"]:
                encoded = generation_wire_json(value)
                self.assertTrue(encoded.startswith('{"kind":'))
                self.assertEqual(json.loads(encoded), value)
        tool = {
            "kind": "TOOL_PROPOSAL",
            "capability": "worktree_read",
            "arguments": {"path": "source.txt", "max_chars": 100},
        }
        self.assertTrue(canonical(tool).startswith('{"arguments":'))
        self.assertEqual(
            generation_wire_json(tool),
    ...
    def test_amendment_contains_changed_runtime_files_and_local_import_dependencies(
        ...
        for name in required:
            for relative in re.findall(
                r"from ['\"]([.][^'\"]+)['\"]", (BASE / name).read_text()
            ):
                target = (
                    ((BASE / name).parent / relative)
                    .resolve()
                    .relative_to(BASE)
                    .as_posix()
                )
