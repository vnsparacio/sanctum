...
class PackageClosure(unittest.TestCase):
    def test_reviewed_overlay_imports_and_builders_without_source_on_import_path(self):
        ...
        required={'worker.py','src/task_evidence.py','runtime/protected-test-driver.cjs','runtime/protected-test-preload.cjs','src/experiment.py','src/experiment_lifecycle.py','experiment_control.py','diagnostic-experiment.mjs','runtime-readiness.mjs','verify_exact_runtime.py','verify_serving_runtime.py','watch.py'}
        self.assertTrue(required<=set(files))
        self.assertEqual(len(files),len(set(files)))
        expected=json.loads(subprocess.check_output(['node',str(ROOT/'gate/runtime-readiness.mjs')]))
        with tempfile.TemporaryDirectory(prefix='sanctum-prelive-package-') as td:
            prefix=Path(td).resolve();package=prefix/'gate';package.mkdir()
            base_files=subprocess.check_output(['git','ls-tree','-r','--name-only','HEAD','gate'],cwd=ROOT,text=True).splitlines()
            for path in base_files:
                name=Path(path).relative_to('gate')
                if 'tests' in name.parts:continue
            ...
            for name in files:
                ...
                tokens={'@STATE@':str(prefix/'state'),'@CONFIG@':str(prefix/'config'),'@GATE@':str(package),'@PYTHON@':sys.executable,'@NODE@':shutil.which('node'),'@OPENCLAW@':str(ROOT/'node_modules/openclaw')}
            ...
            js_edges=0;python_edges=0
            for path in package.rglob('*.mjs'):
                for rel in re.findall(r"(?:from\s*|import\s*\()['\"]([.][^'\"]+)['\"]",path.read_text()):
                    target=(path.parent/rel).resolve();self.assertTrue(target.is_relative_to(package));self.assertTrue(target.exists(),str(target));js_edges+=1
            local={p.stem for p in (package/'src').glob('*.py')}|{p.stem for p in package.glob('*.py')}
            for path in package.rglob('*.py'):
                for node in ast.walk(ast.parse(path.read_text())):
                    ...
                    for name in names:
                        if name and name.split('.')[0] in local:python_edges+=1
...
"""
            subprocess.run([sys.executable,'-B','-I','-c',code,str(package)],cwd=td,check=True,capture_output=True)
            actual=json.loads(subprocess.check_output(['node',str(package/'runtime-readiness.mjs')],cwd=td))
            self.assertEqual(actual,expected)
            # Write only synthetic production requests for the actual isolated CLI.
            (prefix/'input.json').write_text(json.dumps(actual))
            result=subprocess.run([sys.executable,'-B','-I',str(package/'verify_exact_runtime.py'),'--artifact',str(prefix/'input.json')],cwd=td,capture_output=True,text=True)
            self.assertEqual(result.returncode,2);self.assertEqual(json.loads(result.stdout)['status'],'NOT_RUN')
            self.assertGreater(js_edges,40);self.assertGreater(python_edges,45)
            print(f'Package closure: {len(files)} overlay files, {js_edges} JS edges, {python_edges} Python edges; isolated builders identical.')
