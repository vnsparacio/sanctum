...
test('a successful patch makes a host test the only permitted next action',async()=>{
 ...
 const egress=({claim})=>({schema:CONTRACT_VERSION,outcome:'ALLOW',...claim,expires:null,oneUse:false,approvalState:'NONE',reasonCodes:['EXACT_WORK_TASK_EGRESS']});
 const result=await createWorkMode(workConfig({reasoner:reasoner([row(patchTool,{patch:'first'}),row(patchTool,{patch:'second'}),row(testTool,{operation:'test'})]),manifest:tools,workspaceState:async({turn})=>workspaceEvidence({scope:taskId,turn}),invoke:async({proposal})=>{calls.push(proposal.arguments);return {ok:true,executionState:'COMPLETED',verifier:'VERIFIED'};},authorize,egress,evaluate:async()=>({passed:true}),reviewer:null})).run({task:'fix',scope:taskId,requestId,capabilities:[patchTool.name,testTool.name],maxIterations:4});
...
test('a changed host workspace fingerprint rejects a delayed mutation before authority',async()=>{
 ...
 const result=await createWorkMode(workConfig({reasoner:reasoner([{kind:'TOOL_PROPOSAL',capability:patch.name,arguments:{patch:'x'}}]),manifest:m,workspaceState:async({scope,workspace,turn})=>workspaceEvidence({scope,workspace,turn,diffDigest:(++n===1?'d':'f').repeat(64)}),invoke:async()=>({ok:true}),authorize:()=>{authorised++;return {};},egress:defaultResultEgress,evaluate:async()=>({passed:true})})).run({task:'x',scope,requestId,capabilities:[patch.name]});
...
test('patch and test execution facts survive result withholding and unknown completion stops before egress',async()=>{
 const patch={name:'worktree_patch',description:'Patch.',parameters:{type:'object',properties:{task_id:{type:'string'},patch:{type:'string'}},required:['task_id','patch'],additionalProperties:false}};
 ...
 const patchResult=await createWorkMode(workConfig({reasoner:{async invoke(request){requests.push(request);return requests.length===1?proposal('worktree_patch',{patch:'x'}):{kind:'ESCALATION',reason:'STOP_AFTER_ACCOUNTING'};}},manifest:tools,invoke:async()=>({ok:true,executionState:'COMPLETED',verifier:'VERIFIED'}),authorize:allowAuthority,egress:()=>{egressCalls++;return {};},evaluate:async()=>({passed:true})})).run({task:'patch',scope,requestId,capabilities:[patch.name,command.name]});
 ...
 let unknownEgress=0;const unknown=await createWorkMode(workConfig({reasoner:reasoner([proposal('worktree_patch',{patch:'x'})]),manifest:tools,invoke:async()=>({ok:false,error:{code:'UNKNOWN'},executionState:'COMPLETION_UNKNOWN'}),authorize:allowAuthority,egress:()=>{unknownEgress++;return {};},evaluate:async()=>({passed:true})})).run({task:'patch',scope,requestId,capabilities:[patch.name,command.name]});
