...
import {createWorkMode,defaultResultEgress,selectCapabilities} from '../plugin/work-mode.mjs';
...
const run=({rows,invoke=async()=>({ok:true,data:{value:4},verifier:'VERIFIED'}),egress=defaultResultEgress,reviewer=null,authorize, maxIterations=8}={})=>createWorkMode({reasoner:reasoner(rows),manifest,invoke,egress,reviewer,authorize}).run({task:'synthetic task',scope:'a'.repeat(32),requestId:'r'.repeat(32),maxIterations});
...
test('one separate reviewer can require revision or reject before completion',async()=>{
 let reviews=0;const revise=await run({rows:[final,final],reviewer:async()=>{reviews++;return {verdict:reviews===1?'REVISE':'ACCEPT'};}});assert.equal(revise.status,'COMPLETE');assert.equal(reviews,2);
