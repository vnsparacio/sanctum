...
root=Path(FIXTURE_ROOT);source=Path(SOURCE_ROOT)
...
class SyntheticLifecycle:
 def propose(self,scope,request):
  ...
  with patch.object(backend,'health_check'),patch('backends.urllib.request.build_opener') as opener:
   opener.return_value.open.return_value=io.BytesIO((root/'stream.txt').read_bytes())
   return backend.propose(request)

try:
 body=authorize(json.load(sys.stdin),settings,settings_hash='a'*64)
 if body['operation']!='private_lead_propose':raise Refused('fixture_operation')
 print(canonical(execute(body,settings,lifecycle=SyntheticLifecycle())))
except Exception as error:
 result={'status':'UNAVAILABLE','reason':str(error) if type(error) is Refused else 'operation_unavailable'}
 if type(error) is Refused and hasattr(error,'diagnostic'):result['diagnostic']=safe_diagnostic(error.diagnostic)
