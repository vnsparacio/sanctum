...
import backends  # noqa: F401 - retained package edge and patch target
...
class Opener:
    def open(self, req, timeout=None):
        if req.full_url.endswith("/models"):
            return io.BytesIO(
                json.dumps(
                    {"data": [{"id": settings["private_lead"]["alias"]}]}
                ).encode()
            )
        ...
        snapshot = ledger.snapshot(
            {
                k: record[k]
                for k in ("experiment_id", "source_id", "install_id", "deadline")
            }
        )
        with (fixture / "dispatch-counts.jsonl").open("a") as out:
            out.write(
                json.dumps(
                    {
                        "reserved": snapshot["counts"]["reserved"],
