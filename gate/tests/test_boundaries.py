...
class WebUI(unittest.TestCase):
    def setUp(self):self.pipe=module('pipe10',BASE/'webui/pipe.py');self.guard=module('guard10',BASE/'webui/guard.py')
    ...
    def test_file_tool_and_external_feature_ingress_blocked(self):
        for k,v in [('files',[{'id':'x'}]),('tools',{'x':{}}),('features',{'web_search':True})]:
            body={'messages':[{'role':'user','content':'/gate ask synthetic'}],k:v}
            with self.assertRaises(ValueError):asyncio.run(self.guard.Filter().inlet(body))
    def test_memory_removed_before_preprocessing(self):
        body={'messages':[{'role':'user','content':'/gate ask synthetic'}],'features':{'memory':True}};meta={'features':{'memory':True}}
        asyncio.run(self.guard.Filter().inlet(body,meta));self.assertFalse(body['features']['memory']);self.assertFalse(meta['features']['memory'])
    def test_browser_session_disables_implicit_webui_tools(self):
        body={'messages':[{'role':'user','content':'/gate help'}],'params':{'temperature':0}}
        meta={'session_id':'browser-session','params':{'function_calling':'native'}}
        asyncio.run(self.guard.Filter().inlet(body,meta))
        self.assertEqual(body['params'],{'temperature':0,'function_calling':'legacy'})
        self.assertEqual(meta['params']['function_calling'],'legacy')
        body['tools']=[{'type':'function'}]
        with self.assertRaises(ValueError):asyncio.run(self.guard.Filter().inlet(body,meta))
    def test_model_media_is_not_a_user_command(self):
        with self.assertRaises(ValueError):asyncio.run(self.guard.Filter().inlet({'messages':[{'role':'user','content':[{'type':'image_url','image_url':{'url':'https://example.invalid'}}]}]}))
    def test_result_retry_never_replays_approval(self):
        invoke=AsyncMock(side_effect=[ValueError('connection'), 'completed answer'])
        with patch.object(self.pipe,'invoke',invoke),patch.object(self.pipe.asyncio,'sleep',AsyncMock()):
            result=asyncio.run(self.pipe.poll_result({'session':'s','command':'/gate approve secret'},'a'*32))
        self.assertEqual(result,'completed answer');self.assertEqual(invoke.call_count,2)
        self.assertTrue(all(c.args[0]=={'session':'s','command':'/gate result '+'a'*32} for c in invoke.call_args_list))
    def test_result_retry_failure_preserves_job_id(self):
        invoke=AsyncMock(side_effect=ValueError('connection'))
        with patch.object(self.pipe,'invoke',invoke),patch.object(self.pipe.asyncio,'sleep',AsyncMock()):
            result=asyncio.run(self.pipe.poll_result({'session':'s','command':'/gate approve secret'},'b'*32))
        self.assertEqual(invoke.call_count,3);self.assertIn('/gate result '+'b'*32,result);self.assertNotIn('secret',result)

class WorkerSubprocess(unittest.TestCase):
    def test_actual_node_signer_python_worker_and_durable_replay(self):
        with tempfile.TemporaryDirectory() as td:
            root=Path(td);root.chmod(0o700);build=root/'build';build.mkdir();state=root/'state';state.mkdir(mode=0o700)
            shutil.copytree(BASE/'src',build/'src',ignore=shutil.ignore_patterns('__pycache__'))
            shutil.copyfile(BASE/'worker.py',build/'worker.py');(build/'runtime').mkdir();shutil.copyfile(BASE/'runtime/RUNPODCTL.json',build/'runtime/RUNPODCTL.json')
            cfg=settings(state);(build/'SETTINGS.json').write_text(json.dumps(cfg));key=os.urandom(32);(state/'authority.key').write_bytes(key);(state/'authority.key').chmod(0o600)
            files=[p for p in build.rglob('*') if p.is_file()];(build/'FREEZE.json').write_text(json.dumps({str(p.relative_to(build)):hashlib.sha256(p.read_bytes()).hexdigest() for p in files}))
