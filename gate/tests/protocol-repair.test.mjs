...
test('signed worker microprobes use production requests, parser, adapter and host checks',async()=>{
 ...
 try{
  for(const probe of ['inspection','postPatch','inability']){
   ...
   else if(probe==='postPatch'){assert.equal(actions,2);assert.equal(work.status,'COMPLETE');assert.deepEqual(requests[0].state.workIntent,preflightCurrentWorkIntentSchemas().schemas.ordinaryIneligible.request);}
