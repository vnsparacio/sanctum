...
class Filter:
    async def inlet(self, body: dict, __metadata__: dict=None, __user__: dict=None) -> dict:
        # Attached only to the Mac gate model, not ordinary OpenClaw chats.
        metadata = __metadata__ or {}
        if any(body.get(k) or metadata.get(k) for k in ('files','tools','tool_ids','terminal_id','skill_ids','folder_id')):
            raise ValueError('Use local attachment tokens for media. Remove WebUI uploads, tools, skills and project context.')
        ...
        for source in (body, metadata):
            source['params'] = {**(source.get('params') or {}), 'function_calling': 'legacy'}
        ...
        if any((body.get('features') or {}).values()) or any((metadata.get('features') or {}).values()):
            raise ValueError('Turn off search, voice and other extra features for Mac gate. WebUI memory is disabled automatically for this model.')
        rows = body.get('messages', [])
        if any(row.get('role') == 'tool' or row.get('tool_calls') or not isinstance(row.get('content', ''), str) for row in rows):
            raise ValueError('Use a new plain-text Mac gate chat without files or tool results.')
        return body
