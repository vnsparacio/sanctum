...
def launch_contract(text):
    ...
    for key in ("BASE", "CACHE", "MODEL", "REVISION", "ALIAS", "VLLM"):
        ...
        require(len(parts) == 1, "BOOTSTRAP_CONTRACT")
        value = parts[0]
        for name, prior in values.items():
            value = value.replace("$" + name, prior)
    ...
    for i, arg in enumerate(args):
        ...
        require("$" not in arg and "`" not in arg, "BOOTSTRAP_CONTRACT")
        args[i] = arg
...
def verify_command(command, contract):
    ...
    require(
        command == expected
        or (
            len(command) == len(expected) + 1
            and command[1:] == expected
            and Path(command[0]).parent == Path(expected[0]).parent
            and re.fullmatch(r"python(?:3(?:\.[0-9]+)?)?", Path(command[0]).name)
        ),
...
def bind_cache(contract, server_environment, environ=None, modules=None):
    ...
    require(
        not any(
            x in modules for x in ("huggingface_hub.constants", "transformers", "vllm")
        ),
        "CACHE_IMPORT_ORDER",
    )
    home = contract["values"]["CACHE"]
    hub = home + "/hub"
    require(server_environment.get("HF_HOME") == home, "SERVER_CACHE_IDENTITY")
    require(
        all(
            server_environment.get(k) == "1"
            for k in ("HF_HUB_OFFLINE", "TRANSFORMERS_OFFLINE")
        ),
        "SERVER_OFFLINE_REQUIRED",
    )
    ...
    require(
        server_environment.get("HF_ENDPOINT", "https://huggingface.co")
    ...
    return (
        Path(hub)
        / ("models--" + contract["values"]["MODEL"].replace("/", "--"))
        / "snapshots"
...
def verify_model_identity(
    requested_model, requested_revision, resolved_model, contract, snapshot
):
    values = contract["values"]
    require(
        requested_model == values["MODEL"] and requested_revision == values["REVISION"],
        "MODEL_REQUEST_IDENTITY",
    )
    require(
        snapshot.is_dir()
        and not snapshot.is_symlink()
        and snapshot.name == values["REVISION"],
        "CACHED_SNAPSHOT_ABSENT",
    )
    require(
        Path(resolved_model).is_absolute()
        and Path(resolved_model).resolve() == snapshot.resolve(),
...
def failure_record(error, stage):
    ...
    frames = [
        {"module": Path(f.filename).name, "line": f.lineno, "function": f.name}
        for f in traceback.extract_tb(error.__traceback__)[-8:]
    ]
    code = (
        str(error)
        if type(error) is VerificationFailure
        and re.fullmatch("[A-Z0-9_]{1,80}", str(error))
        else "EXACT_RUNTIME_CHECK_FAILED"
    )
    return {
        "schema": "sanctum-serving-runtime-verification/v1",
        "status": "FAIL",
        "stage": stage,
        "code": code,
...
def process_facts(root, proc=Path("/proc")):
    ...
    command = [
        x.decode() for x in (directory / "cmdline").read_bytes().split(b"\0") if x
    ]
    ...
    for entry in (directory / "environ").read_bytes().split(b"\0"):
        ...
        if sep and key.decode(errors="replace") in CACHE_KEYS:
...
def run(base=BASE, proc=Path("/proc"), progress=None):
    ...
    try:
        ...
        root = Path(contract["values"]["BASE"])
        pid, start, command, environment = process_facts(root, proc)
        ...
        requested = args.model_tag or args.model
        ...
        if engine.reasoning_parser:
            config.reasoning_parser = engine.reasoning_parser
        require(
            config.backend == "auto" and config.disable_any_whitespace is False,
            "BACKEND_CONFIG_DRIFT",
        )
        mark("TOKENIZER_IDENTITY")
        expected = strict_json((base / "expected-tokenizer.json").read_text())
        for name, sha in expected["files"].items():
            require(
                name in exact.TOKEN_FILES and exact.sha(snapshot / name) == sha,
                "TOKENIZER_BYTES_MISMATCH",
            )
        require(
            {"config.json", "tokenizer.json", "tokenizer_config.json"}
            <= set(expected["files"]),
            "TOKENIZER_EXPECTATION_INCOMPLETE",
        )
        ...
        artifact = strict_json((base / "runtime-requests.json").read_text())
        exact.verify_artifact(artifact)
        mark("BACKEND_RESOLUTION")
        selected = {}
        for label, row in artifact["surfaces"].items():
            params = SamplingParams(
                structured_outputs=StructuredOutputsParams(
                    json=exact.generation_order(row["request"]["schema"])
                )
            )
        ...
        resolution = {
            ...
            "selection_config": dataclasses.asdict(config),
            "per_surface_backend": selected,
            "sampling_params_sha256": exact.sha(
                Path(importlib.import_module("vllm.sampling_params").__file__)
            ),
            "cacheBindingVerified": True,
        ...
        resolved = {
            ...
            "selection_config": dataclasses.asdict(config),
            "per_surface_backend": selected,
            "resolution_evidence_sha256": digest(resolution),
        }
        mark("ENVIRONMENT_INVENTORY")
        observed, _ = exact.inventory(snapshot, resolved)
        observed["artifactDigest"] = digest(artifact)
        mark("SCHEMA_AND_TOKEN_VERIFICATION")
        result = exact.run(artifact, observed, snapshot)
        result["resolutionEvidence"] = resolution
        mark("NEGATIVE_COMPILER_CONTROLS")
        negative = []
        model = strict_json((snapshot / "config.json").read_text())
        vocab = model.get("vocab_size", model.get("text_config", {}).get("vocab_size"))
        cfg = SimpleNamespace(structured_outputs_config=config, speculative_config=None)
        for label, row in artifact["surfaces"].items():
            module, cls = exact.BACKENDS[selected[label]]
            backend = getattr(
                importlib.import_module("vllm.v1.structured_output." + module), cls
            )(cfg, tokenizer, vocab)
            try:
                grammar = backend.compile_grammar(
                    StructuredOutputOptions.JSON,
                    exact.generation_wire_json(row["request"]["schema"]),
                )
                accepted = all(
                    grammar.accept_tokens("invalid", [t])
                    for t in tokenizer.encode("{}", add_special_tokens=False)
                )
                if accepted:
                    accepted = grammar.accept_tokens(
                        "invalid", [tokenizer.eos_token_id]
                    )
                refused = False
                try:
                    backend.compile_grammar(
                        StructuredOutputOptions.JSON, '{"type":"INVALID_TYPE"}'
                    )
                except Exception:
                    refused = True
                negative.append(
                    {
                        "surface": label,
                        "invalidRepresentativeRefused": not accepted,
                        "invalidSchemaRefused": refused,
                    }
                )
            finally:
                backend.destroy()
        ...
        if not all(
            x["invalidRepresentativeRefused"] and x["invalidSchemaRefused"]
            for x in negative
        ):
        ...
        require(
            (pid, start, command, environment)
            == (end_pid, end_start, end_command, end_env),
            "SERVER_IDENTITY_CHANGED",
        )
        result.update(stage="COMPLETE", serverIdentityStable=True)
        return result
    except BaseException as error:
        return failure_record(error, stage)
