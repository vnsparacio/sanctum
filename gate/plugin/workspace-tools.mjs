...
export const workModeTools=Object.freeze([
 ...
 Object.freeze({name:'worktree_edit',description:'Perform one bounded file action in the isolated workspace using exactly one shape: replace {path,old_text,new_text} with no operation; create {operation:create,path,new_text}; delete {operation:delete,path}; move {operation:move,path,destination}. Do not include fields from another shape. Create uses new_text as the complete UTF-8 file and refuses an existing path. Replace, delete and move require reading the source first. The Mac checks protected paths, symlinks, and scope, then generates the exact Git diff.',parameters:object({task_id:task,operation:{type:'string',enum:['create','delete','move']},path:relative,destination:relative,old_text:{type:'string',minLength:1,maxLength:4096},new_text:{type:'string',maxLength:8192}},['task_id','path'])}),
