...
export default {
  ...
  register(api){
    ...
    const invokeWeb=async(name,args,proposal)=>{
      ...
      if(!api.capabilityManifest||gateway?.bind!=='loopback'||gateway?.auth?.mode!=='token'||!gateway.auth.token)throw Error('source_runtime_unavailable');
    ...
    const retrieval=createSourceRetrieval({manifest:api.capabilityManifest,invoke:invokeWeb});
    const gate=createGate({settings,key,execute:(body,signal)=>body.operation==='answer_local'?local(body,signal):remote(body,signal),retrieve:request=>retrieval.retrieve(request)});
