...
REASONS = {
    ...
    "TOOL_INCOMPATIBILITY_REPEATED",
    "VERIFICATION_FAILED",
    "TOOL_PLAN_FAILED",
    "COMPLEX_STRUCTURED_TASK",
    "SEMANTIC_QUALITY_REQUIRED",
}
...
def decide(context, *, reason=None, incompatibilities=0, provider_config=None):
    ...
    if context.current_tier not in ("4b", "80b", "235b"):
        raise ValueError("invalid tier")
    sources = list(context.sources) or ["unknown"]
    if context.session_privacy != "CLEAN":
        sources.append(
            "credentials" if context.session_privacy == "RESTRICTED" else "unknown"
        )
    task = context.task
    if reason in ("TOOL_VALIDATION_FAILED", "TOOL_PLAN_FAILED", "VERIFICATION_FAILED"):
        task = "analyze"
    if reason == "TOOL_INCOMPATIBILITY_REPEATED" and incompatibilities < 2:
        reason = None
    ...
    if reason is None and decision["difficulty"] != "EASY":
        reason = (
            "SEMANTIC_QUALITY_REQUIRED"
            if decision["difficulty"] == "VERY_HARD"
            else "COMPLEX_STRUCTURED_TASK"
        )
    required = reason is not None
    if not required:
        target = None
    elif context.current_tier == "235b":
        target = None
    elif (
        context.current_tier == "80b"
        or reason == "SEMANTIC_QUALITY_REQUIRED"
        or decision["difficulty"] == "VERY_HARD"
    ):
        target = "235b"
    else:
        target = "80b"
    privacy_allowed = (
        decision["hosted_egress_allowed"]
        and decision["privacy"] == "PUBLIC"
        and context.session_privacy == "CLEAN"
    )
    ...
    outcome = (
        ...
        if not required
            ...
            if target is None
                ...
                else "PROVIDER_NOT_CONFIGURED" if not configured else "DRY_RUN_READY"
            )
        )
    )
    return {
        "initial_model": "qwen-4b",
        ...
        print(
            json.dumps(
                decide(
                    context,
                    reason=event["reason"],
                    incompatibilities=event.get("incompatibilities", 0),
                )
            )
        )
    except Exception:
            ...
            json.dumps(
                {
                    ...
                    "escalated": False,
                    "remote_call_permitted": False,
                    "final_outcome": "POLICY_UNAVAILABLE",
                }
            )
        )
