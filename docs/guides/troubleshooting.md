...
| Local answer unavailable | Confirm MLX health, run doctor, and confirm Open WebUI is using **Mac prompt gate**, not the raw MLX provider. Bare-model health does not establish the authenticated agent path. After a Work Mode upgrade, reapply the supported amendment so `main` owns local requests and ambient model resolution. |
