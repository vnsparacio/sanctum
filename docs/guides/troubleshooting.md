...
| Local tool answer ends with `Reply truncated at the model's output token limit` after only a fragment | Stop the gateway, apply the current `scripts/upgrade_work_mode.py` amendment, run `make doctor PREFIX=...`, and restart. Current source pins `main.thinkingDefault` to `off` and sends `chat_template_kwargs.enable_thinking=false` to MLX so hidden reasoning cannot consume the final-answer budget; Gate refuses a local handoff if either pin is absent. Do not route personal tool results to a hosted model as a workaround. |
