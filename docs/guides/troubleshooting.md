...
| Local tool answer ends with `Reply truncated at the model's output token limit` after only a fragment | Confirm the installed Gate local adapter matches current reviewed source and `make doctor PREFIX=...` passes. Older V1.3 candidates capped the entire local agent/tool loop below the reviewed `max_answer_tokens` setting, which could leave only one token for the final answer. Do not route personal tool results to a hosted model as a workaround. |
