...
In WebUI Functions, import the rendered prefix `gate/webui/guard.py` as the gate boundary filter and `gate/webui/pipe.py` as its pipe. Enable the filter on the gate model. Use a saved administrator chat, not a temporary chat. Ordinary text enters Assistant Mode directly; `/gate new` remains available to clear the conversation, and `/gate help` shows controls. When disclosure is needed, the pipe uses authenticated Open WebUI confirmation dialogs for send-once, the narrowly bounded Gemini audit session grant, or keep-local/deny. `/gate audit status` shows the grant and `/gate audit revoke` removes it. Work Mode still requires an explicit `/work` command. Upload/RAG/search/memory preprocessing must remain blocked. The host bridge and gateway use the generated private configuration; copying the pipe from the unrendered source will not work.
