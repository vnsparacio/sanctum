...
For current public requests, Source-First keeps a public `today` cue in its minimized search query. A single ZIP explicitly supplied for a non-private weather request is retained as the requested location; unrelated numeric identifiers and ZIPs in private context remain omitted. For that weather case, candidates must identify the ZIP and fetched content must contain forecast facts before evidence is marked adequate. Search/fetch success alone still does not guarantee an answer: the Mac gate requires a grounded, delivered citation for `WEB_REQUIRED` requests and refuses an unsupported forecast.
