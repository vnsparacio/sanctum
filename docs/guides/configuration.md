...
For current public requests, Source-First keeps a public `today` cue in its minimized search query but omits bare numeric ZIP codes. Candidate ordering rewards matching multiword subject and location phrases, so an unrelated government result is less likely to outrank a location-matching result. Search/fetch success still does not guarantee an answer: the Mac gate requires a grounded, delivered citation for `WEB_REQUIRED` requests and refuses an unsupported forecast.
