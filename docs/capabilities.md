...
The capability manifest is a runtime projection, not an authority list: it separately records whether a tool is declared, its schema was captured, configuration exposes it, and a run may use it. Schema capture or a tool description never grants permission. A declared tool whose schema is missing is non-exposed and fails closed. The shared proposal/result contracts retain existing bounded repair, provenance, untrusted markers, result truncation and rollback behavior; they do not broaden any capability.
