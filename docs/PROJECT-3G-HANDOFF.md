...
Host-owned completion eligibility is installed, and the ordinary-ineligible, test-only-ineligible, ordinary-eligible and research-ineligible schemas all work on the pinned live endpoint. The F-D harness correction passes offline but is not installed, and F-C produced no 11-case evidence. Project 3G therefore remains unaccepted. Do not promote PRIVATE_LEAD, switch ownership permanently, remove the 80B rollback path, run another paid suite, open a PR, or begin Project 3H from this state. A separately authorized final qualification must install the exact F-D commit, pass the new Mac-side gateway precondition before allocation, and preserve the unchanged suite and accepted security/model contracts.
